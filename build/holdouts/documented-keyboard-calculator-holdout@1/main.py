...
import operator
import sys
IDENTITY = 'documented-keyboard-calculator-holdout'
...
SELF_TEST_CONTROLS = [{'identity': 'command.clear.compact', 'label': 'C', 'row': 2, 'column': 0, 'action': 'clear'}, {'identity': 'command.backspace', 'label': '⌫', 'row': 2, 'column': 1, 'action': 'backspace'}, {'identity': 'operator.expression.divide', 'label': '÷', 'row': 2, 'column': 3, 'action': 'append', 'value': '/'}, {'identity': 'digit.7', 'label': '7', 'row': 3, 'column': 0, 'action': 'append', 'value': '7'}, {'identity': 'digit.8', 'label': '8', 'row': 3, 'column': 1, 'action': 'append', 'value': '8'}, {'identity': 'digit.9', 'label': '9', 'row': 3, 'column': 2, 'action': 'append', 'value': '9'}, {'identity': 'operator.expression.multiply', 'label': '×', 'row': 3, 'column': 3, 'action': 'append', 'value': '*'}, {'identity': 'digit.4', 'label': '4', 'row': 4, 'column': 0, 'action': 'append', 'value': '4'}, {'identity': 'digit.5', 'label': '5', 'row': 4, 'column': 1, 'action': 'append', 'value': '5'}, {'identity': 'digit.6', 'label': '6', 'row': 4, 'column': 2, 'action': 'append', 'value': '6'}, {'identity': 'operator.expression.subtract', 'label': '−', 'row': 4, 'column': 3, 'action': 'append', 'value': '-'}, {'identity': 'digit.1', 'label': '1', 'row': 5, 'column': 0, 'action': 'append', 'value': '1'}, {'identity': 'digit.2', 'label': '2', 'row': 5, 'column': 1, 'action': 'append', 'value': '2'}, {'identity': 'digit.3', 'label': '3', 'row': 5, 'column': 2, 'action': 'append', 'value': '3'}, {'identity': 'operator.expression.add', 'label': '+', 'row': 5, 'column': 3, 'action': 'append', 'value': '+'}, {'identity': 'digit.0', 'label': '0', 'row': 6, 'column': 0, 'action': 'append', 'value': '0'}, {'identity': 'syntax.decimal', 'label': '.', 'row': 6, 'column': 2, 'action': 'append', 'value': '.'}, {'identity': 'command.evaluate', 'label': '=', 'row': 6, 'column': 3, 'action': 'evaluate'}]

def build_interface():
    global display, mode_text, Button, Entry, Label, StringVar, Tk
    from tkinter import Button, Entry, Label, StringVar, Tk
    root = Tk()
    root.title('Documented Keyboard Calculator — Reviewed Holdout')
    root.geometry('400x600+0+0')
    root.configure(bg='#f0f0f0')
    display = StringVar()
    mode_text = StringVar(value='reviewed holdout · documented desktop keyboard projection')
    Entry(root, textvariable=display, font=('Menlo', 18), justify='right').grid(row=0, column=0, columnspan=4, sticky='nsew')
    Label(root, textvariable=mode_text, bg='#f0f0f0', fg='#000000').grid(row=1, column=0, columnspan=4, sticky='w')
    Button(root, text='C', command=clear, bg='#ffffff', fg='#000000', width=7).grid(row=2, column=0, sticky='nsew')
    Button(root, text='⌫', command=backspace, bg='#ffffff', fg='#000000', width=7).grid(row=2, column=1, sticky='nsew')
    Button(root, text='÷', command=lambda value='/': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=2, column=3, sticky='nsew')
    Button(root, text='7', command=lambda value='7': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=3, column=0, sticky='nsew')
    Button(root, text='8', command=lambda value='8': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=3, column=1, sticky='nsew')
    Button(root, text='9', command=lambda value='9': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=3, column=2, sticky='nsew')
    Button(root, text='×', command=lambda value='*': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=3, column=3, sticky='nsew')
    Button(root, text='4', command=lambda value='4': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=4, column=0, sticky='nsew')
    Button(root, text='5', command=lambda value='5': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=4, column=1, sticky='nsew')
    Button(root, text='6', command=lambda value='6': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=4, column=2, sticky='nsew')
    Button(root, text='−', command=lambda value='-': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=4, column=3, sticky='nsew')
    Button(root, text='1', command=lambda value='1': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=5, column=0, sticky='nsew')
    Button(root, text='2', command=lambda value='2': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=5, column=1, sticky='nsew')
    Button(root, text='3', command=lambda value='3': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=5, column=2, sticky='nsew')
    Button(root, text='+', command=lambda value='+': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=5, column=3, sticky='nsew')
    Button(root, text='0', command=lambda value='0': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=6, column=0, sticky='nsew')
    Button(root, text='.', command=lambda value='.': append(value), bg='#ffffff', fg='#000000', width=7).grid(row=6, column=2, sticky='nsew')
    Button(root, text='=', command=evaluate, bg='#ffffff', fg='#000000', width=7).grid(row=6, column=3, sticky='nsew')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('reviewed holdout · documented desktop keyboard projection')
...
def self_test_interface(root):
    ...
    display.set('10+5')
    state['expression'] = ''
    widgets = [item for item in root.grid_slaves(row=6, column=3) if item.winfo_class() == 'Button']
    widgets[0].invoke()
    results.append(display.get() == '15')
    reset_interface()
