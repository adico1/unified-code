...
APPLICATION_ID = 'uc://applications/multiball-paddle-duel@1'
...
INITIAL_STATE = {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': 5, 'vy': 3, 'width': 10, 'x': 195, 'y': 115}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}, 'orb-2': {'height': 10, 'vx': -4, 'vy': -2, 'width': 10, 'x': 220, 'y': 140}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 0}
...
def advance():
    ...
    state['entities']['orb-2']['x'] += state['entities']['orb-2']['vx']
    state['entities']['orb-2']['y'] += state['entities']['orb-2']['vy']
    ...
    if state['entities']['orb']['y'] > 230:
        ...
        state['entities']['orb']['vy'] = -abs(state['entities']['orb']['vy'])
    if state['entities']['orb-2']['y'] < 0:
        state['entities']['orb-2']['y'] = 0
        state['entities']['orb-2']['vy'] = abs(state['entities']['orb-2']['vy'])
    if state['entities']['orb-2']['y'] > 230:
        state['entities']['orb-2']['y'] = 230
        state['entities']['orb-2']['vy'] = -abs(state['entities']['orb-2']['vy'])
    ...
    if (state['entities']['orb']['x'] < state['entities']['right-bat']['x'] + state['entities']['right-bat']['width'] and state['entities']['orb']['x'] + state['entities']['orb']['width'] > state['entities']['right-bat']['x'] and state['entities']['orb']['y'] < state['entities']['right-bat']['y'] + state['entities']['right-bat']['height'] and state['entities']['orb']['y'] + state['entities']['orb']['height'] > state['entities']['right-bat']['y']):
        state['entities']['orb']['vx'] *= -1
    if (state['entities']['orb-2']['x'] < state['entities']['left-bat']['x'] + state['entities']['left-bat']['width'] and state['entities']['orb-2']['x'] + state['entities']['orb-2']['width'] > state['entities']['left-bat']['x'] and state['entities']['orb-2']['y'] < state['entities']['left-bat']['y'] + state['entities']['left-bat']['height'] and state['entities']['orb-2']['y'] + state['entities']['orb-2']['height'] > state['entities']['left-bat']['y']):
        state['entities']['orb-2']['vx'] *= -1
    if (state['entities']['orb-2']['x'] < state['entities']['right-bat']['x'] + state['entities']['right-bat']['width'] and state['entities']['orb-2']['x'] + state['entities']['orb-2']['width'] > state['entities']['right-bat']['x'] and state['entities']['orb-2']['y'] < state['entities']['right-bat']['y'] + state['entities']['right-bat']['height'] and state['entities']['orb-2']['y'] + state['entities']['orb-2']['height'] > state['entities']['right-bat']['y']):
        state['entities']['orb-2']['vx'] *= -1
    ...
    if state['entities']['orb']['x'] > 390:
        state['left_score'] += 1
        state['entities']['orb']['x'] = 195
    if state['entities']['orb-2']['x'] < 0:
        state['right_score'] += 1
        state['entities']['orb-2']['x'] = 220
    if state['entities']['orb-2']['x'] > 390:
        state['left_score'] += 1
        state['entities']['orb-2']['x'] = 220
    state['tick'] += 1
    present_state()
...
def present_state():
    ...
    _surface.create_rectangle(state['entities']['orb-2']['x'], state['entities']['orb-2']['y'], state['entities']['orb-2']['x'] + state['entities']['orb-2']['width'], state['entities']['orb-2']['y'] + state['entities']['orb-2']['height'], fill='cyan', outline='cyan')
...
def build_interface():
    ...
    _root = Tk()
    _root.title('Multiball Paddle Duel')
    ...
    _buttons['participant.right.up'] = Button(_root, text='Right ↑', command=control_2)
    _buttons['participant.right.up'].grid(row=1, column=2, sticky='nsew')
    _root.bind('<Up>', lambda event, operation=control_2: operation())
    _buttons['participant.right.down'] = Button(_root, text='Right ↓', command=control_3)
    _buttons['participant.right.down'].grid(row=1, column=3, sticky='nsew')
...
def run_case(case):
    ...
    operations = {
        ...
        'participant.right.up': control_2,
        'participant.right.down': control_3,
    }
...
def run_acceptance():
    cases = [{'expected': {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': 5, 'vy': 3, 'width': 10, 'x': 200, 'y': 118}, 'orb-2': {'height': 10, 'vx': -4, 'vy': -2, 'width': 10, 'x': 216, 'y': 138}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 1}, 'id': 'multiball.parallel-motion', 'input': {'steps': [{'ticks': 1}]}}]
...
def self_test_application():
    ...
    root.withdraw()
    cases = [{'expected': {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': 5, 'vy': 3, 'width': 10, 'x': 200, 'y': 118}, 'orb-2': {'height': 10, 'vx': -4, 'vy': -2, 'width': 10, 'x': 216, 'y': 138}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 1}, 'input': {'steps': [{'ticks': 1}]}}]
    checks = []
    for case in cases:
        reset_state()
        ...
        checks.append(len(_surface.find_all()) == 5)
    root.destroy()
