...
APPLICATION_ID = 'uc://applications/power-up-paddle-arena@1'
...
INITIAL_STATE = {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': 5, 'vy': 3, 'width': 10, 'x': 180, 'y': 115}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}, 'power-zone': {'height': 100, 'vx': 0, 'vy': 0, 'width': 20, 'x': 200, 'y': 70}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 0}
...
def advance():
    ...
    if (state['entities']['orb']['x'] < state['entities']['right-bat']['x'] + state['entities']['right-bat']['width'] and state['entities']['orb']['x'] + state['entities']['orb']['width'] > state['entities']['right-bat']['x'] and state['entities']['orb']['y'] < state['entities']['right-bat']['y'] + state['entities']['right-bat']['height'] and state['entities']['orb']['y'] + state['entities']['orb']['height'] > state['entities']['right-bat']['y']):
        state['entities']['orb']['vx'] *= -1
    if (state['entities']['orb']['x'] < state['entities']['power-zone']['x'] + state['entities']['power-zone']['width'] and state['entities']['orb']['x'] + state['entities']['orb']['width'] > state['entities']['power-zone']['x'] and state['entities']['orb']['y'] < state['entities']['power-zone']['y'] + state['entities']['power-zone']['height'] and state['entities']['orb']['y'] + state['entities']['orb']['height'] > state['entities']['power-zone']['y']):
        state['entities']['orb']['vx'] *= 1
        state['entities']['orb']['vx'] *= 1.5
...
def present_state():
    ...
    _surface.create_rectangle(state['entities']['power-zone']['x'], state['entities']['power-zone']['y'], state['entities']['power-zone']['x'] + state['entities']['power-zone']['width'], state['entities']['power-zone']['y'] + state['entities']['power-zone']['height'], fill='purple', outline='purple')
...
def build_interface():
    ...
    _root = Tk()
    _root.title('Power-up Paddle Arena')
...
def run_acceptance():
    cases = [{'expected': {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': 7.5, 'vy': 3, 'width': 10, 'x': 195, 'y': 124}, 'power-zone': {'height': 100, 'vx': 0, 'vy': 0, 'width': 20, 'x': 200, 'y': 70}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 3}, 'id': 'power-up.speed-zone', 'input': {'steps': [{'ticks': 3}]}}]
...
def self_test_application():
    ...
    root.withdraw()
    cases = [{'expected': {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': 7.5, 'vy': 3, 'width': 10, 'x': 195, 'y': 124}, 'power-zone': {'height': 100, 'vx': 0, 'vy': 0, 'width': 20, 'x': 200, 'y': 70}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 3}, 'input': {'steps': [{'ticks': 3}]}}]
    checks = []
    for case in cases:
        reset_state()
