...
APPLICATION_ID = 'uc://applications/wall-return-training@1'
...
def advance():
    ...
    if state['entities']['orb']['x'] < 0:
        state['entities']['orb']['x'] = 0
        state['entities']['orb']['vx'] = abs(state['entities']['orb']['vx'])
    if state['entities']['orb']['x'] > 390:
        state['entities']['orb']['x'] = 390
        state['entities']['orb']['vx'] = -abs(state['entities']['orb']['vx'])
...
def build_interface():
    ...
    _root = Tk()
    _root.title('Wall-return Training')
...
def run_acceptance():
    cases = [{'expected': {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': -5, 'vy': -3, 'width': 10, 'x': 390, 'y': 227}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 40}, 'id': 'wall-training.reflection', 'input': {'steps': [{'ticks': 40}]}}]
...
def self_test_application():
    ...
    root.withdraw()
    cases = [{'expected': {'entities': {'left-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 20, 'y': 90}, 'orb': {'height': 10, 'vx': -5, 'vy': -3, 'width': 10, 'x': 390, 'y': 227}, 'right-bat': {'height': 60, 'vx': 0, 'vy': 0, 'width': 10, 'x': 370, 'y': 90}}, 'left_score': 0, 'right_score': 0, 'status': 'playing', 'tick': 40}, 'input': {'steps': [{'ticks': 40}]}}]
    checks = []
    for case in cases:
        reset_state()
        ...
        checks.append(len(_surface.find_all()) == 3)
    root.destroy()
