...
from file_editor import cli, runtime
...
def test_generated_domain_and_composition_have_no_control_flow():
    import ast
    package_root = Path(__file__).parents[1] / 'file_editor'
...
def test_failed_atomic_persistence_preserves_original(tmp_path):
    case = {'request': {'action': 'replace', 'path': 'draft.txt', 'match': 'alpha', 'value': 'omega', 'save': True}, 'fixture_hex': '616c70686120626574610a'}
...
def run():
    checks = [
        ...
        ("atomic-persistence", lambda root: test_failed_atomic_persistence_preserves_original(root)),
    ]
