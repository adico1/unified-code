...
APPLICATION_ID = 'uc://applications/travel-checklist@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'trip_phase_field', 'packing_state_field', 'document_state_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/travel-checklist/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_TRAVEL_CHECKLIST_STATE'
...
def command_create(arguments):
    ...
    if 'trip_phase_field' not in arguments:
        return _failure('missing-trip_phase_field')
    if 'packing_state_field' not in arguments:
        return _failure('missing-packing_state_field')
    if 'document_state_field' not in arguments:
        return _failure('missing-document_state_field')
    ...
    state['records'].append({'completed': False, 'document_state_field': arguments['document_state_field'], 'id': state['next_id'], 'packing_state_field': arguments['packing_state_field'], 'title': arguments['title'], 'trip_phase_field': arguments['trip_phase_field']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['document_state_field'] = arguments['document_state_field']
            record['packing_state_field'] = arguments['packing_state_field']
            record['title'] = arguments['title']
            record['trip_phase_field'] = arguments['trip_phase_field']
    persist_state()
    present_state()
    return _success()
...
def control_0():
    ...
    _last_outcome = command_create({'document_state_field': _inputs['entry.document_state_field'].get(), 'packing_state_field': _inputs['entry.packing_state_field'].get(), 'title': _inputs['entry.primary'].get(), 'trip_phase_field': _inputs['entry.trip_phase_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'document_state_field': _inputs['entry.document_state_field'].get(), 'id': selected_value('collection.primary', 'id'), 'packing_state_field': _inputs['entry.packing_state_field'].get(), 'title': _inputs['entry.primary'].get(), 'trip_phase_field': _inputs['entry.trip_phase_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Trip Phase Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.trip_phase_field'] = Entry(_root, width=38)
    _inputs['entry.trip_phase_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Packing State Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.packing_state_field'] = Entry(_root, width=38)
    _inputs['entry.packing_state_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Document State Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.document_state_field'] = Entry(_root, width=38)
    _inputs['entry.document_state_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}, 'id': 'travel-checklist.lifecycle', 'input': {'steps': [{'arguments': {'document_state_field': 'document_state_field-value', 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}, 'inputs': {'entry.document_state_field': 'document_state_field-value', 'entry.packing_state_field': 'packing_state_field-value', 'entry.primary': 'A', 'entry.trip_phase_field': 'trip_phase_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'B', 'trip_phase_field': 'trip_phase_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'B', 'trip_phase_field': 'trip_phase_field-value'}]}}, 'inputs': {'entry.document_state_field': 'document_state_field-value', 'entry.packing_state_field': 'packing_state_field-value', 'entry.primary': 'B', 'entry.trip_phase_field': 'trip_phase_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'document_state_field': 'document_state_field-value', 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'document_state_field': 'document_state_field-value', 'id': 1, 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'document_state_field': 'document_state_field-value', 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'document_state_field': 'document_state_field-value', 'packing_state_field': 'packing_state_field-value', 'title': 'A', 'trip_phase_field': 'trip_phase_field-value'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
