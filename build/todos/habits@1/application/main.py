...
APPLICATION_ID = 'uc://applications/habits@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'recurrence_field', 'streak_field', 'daily_observation_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/habits/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_HABITS_STATE'
...
def command_create(arguments):
    ...
    if 'recurrence_field' not in arguments:
        return _failure('missing-recurrence_field')
    if 'streak_field' not in arguments:
        return _failure('missing-streak_field')
    if 'daily_observation_field' not in arguments:
        return _failure('missing-daily_observation_field')
    ...
    state['records'].append({'completed': False, 'daily_observation_field': arguments['daily_observation_field'], 'id': state['next_id'], 'recurrence_field': arguments['recurrence_field'], 'streak_field': arguments['streak_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['daily_observation_field'] = arguments['daily_observation_field']
            record['recurrence_field'] = arguments['recurrence_field']
            record['streak_field'] = arguments['streak_field']
...
def control_0():
    ...
    _last_outcome = command_create({'daily_observation_field': _inputs['entry.daily_observation_field'].get(), 'recurrence_field': _inputs['entry.recurrence_field'].get(), 'streak_field': _inputs['entry.streak_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'daily_observation_field': _inputs['entry.daily_observation_field'].get(), 'id': selected_value('collection.primary', 'id'), 'recurrence_field': _inputs['entry.recurrence_field'].get(), 'streak_field': _inputs['entry.streak_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Recurrence Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.recurrence_field'] = Entry(_root, width=38)
    _inputs['entry.recurrence_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Streak Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.streak_field'] = Entry(_root, width=38)
    _inputs['entry.streak_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Daily Observation Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.daily_observation_field'] = Entry(_root, width=38)
    _inputs['entry.daily_observation_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}, 'id': 'habits.lifecycle', 'input': {'steps': [{'arguments': {'daily_observation_field': 'daily_observation_field-value', 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}, 'inputs': {'entry.daily_observation_field': 'daily_observation_field-value', 'entry.primary': 'A', 'entry.recurrence_field': 'recurrence_field-value', 'entry.streak_field': 'streak_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'B'}]}}, 'inputs': {'entry.daily_observation_field': 'daily_observation_field-value', 'entry.primary': 'B', 'entry.recurrence_field': 'recurrence_field-value', 'entry.streak_field': 'streak_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'daily_observation_field': 'daily_observation_field-value', 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'daily_observation_field': 'daily_observation_field-value', 'id': 1, 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'daily_observation_field': 'daily_observation_field-value', 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'daily_observation_field': 'daily_observation_field-value', 'recurrence_field': 'recurrence_field-value', 'streak_field': 'streak_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
