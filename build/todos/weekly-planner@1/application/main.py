...
APPLICATION_ID = 'uc://applications/weekly-planner@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'weekday_field', 'weekly_order_field', 'carry_forward_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/weekly-planner/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_WEEKLY_PLANNER_STATE'
...
def command_create(arguments):
    ...
    if 'weekday_field' not in arguments:
        return _failure('missing-weekday_field')
    if 'weekly_order_field' not in arguments:
        return _failure('missing-weekly_order_field')
    if 'carry_forward_field' not in arguments:
        return _failure('missing-carry_forward_field')
    ...
    state['records'].append({'carry_forward_field': arguments['carry_forward_field'], 'completed': False, 'id': state['next_id'], 'title': arguments['title'], 'weekday_field': arguments['weekday_field'], 'weekly_order_field': arguments['weekly_order_field']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['carry_forward_field'] = arguments['carry_forward_field']
            record['title'] = arguments['title']
            record['weekday_field'] = arguments['weekday_field']
            record['weekly_order_field'] = arguments['weekly_order_field']
    persist_state()
    present_state()
    return _success()
...
def control_0():
    ...
    _last_outcome = command_create({'carry_forward_field': _inputs['entry.carry_forward_field'].get(), 'title': _inputs['entry.primary'].get(), 'weekday_field': _inputs['entry.weekday_field'].get(), 'weekly_order_field': _inputs['entry.weekly_order_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'carry_forward_field': _inputs['entry.carry_forward_field'].get(), 'id': selected_value('collection.primary', 'id'), 'title': _inputs['entry.primary'].get(), 'weekday_field': _inputs['entry.weekday_field'].get(), 'weekly_order_field': _inputs['entry.weekly_order_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Weekday Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.weekday_field'] = Entry(_root, width=38)
    _inputs['entry.weekday_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Weekly Order Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.weekly_order_field'] = Entry(_root, width=38)
    _inputs['entry.weekly_order_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Carry Forward Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.carry_forward_field'] = Entry(_root, width=38)
    _inputs['entry.carry_forward_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': False, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': True, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': True, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': True, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, 'id': 'weekly-planner.lifecycle', 'input': {'steps': [{'arguments': {'carry_forward_field': 'carry_forward_field-value', 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': False, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': False, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, 'inputs': {'entry.carry_forward_field': 'carry_forward_field-value', 'entry.primary': 'A', 'entry.weekday_field': 'weekday_field-value', 'entry.weekly_order_field': 'weekly_order_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': False, 'id': 1, 'title': 'B', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': False, 'id': 1, 'title': 'B', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, 'inputs': {'entry.carry_forward_field': 'carry_forward_field-value', 'entry.primary': 'B', 'entry.weekday_field': 'weekday_field-value', 'entry.weekly_order_field': 'weekly_order_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'carry_forward_field': 'carry_forward_field-value', 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': True, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'carry_forward_field': 'carry_forward_field-value', 'completed': True, 'id': 1, 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'carry_forward_field': 'carry_forward_field-value', 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'carry_forward_field': 'carry_forward_field-value', 'title': 'A', 'weekday_field': 'weekday_field-value', 'weekly_order_field': 'weekly_order_field-value'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
