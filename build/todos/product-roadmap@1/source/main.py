...
APPLICATION_ID = 'uc://applications/product-roadmap@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'initiative_field', 'target_window_field', 'status_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/product-roadmap/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_PRODUCT_ROADMAP_STATE'
...
def command_create(arguments):
    ...
    if 'initiative_field' not in arguments:
        return _failure('missing-initiative_field')
    if 'target_window_field' not in arguments:
        return _failure('missing-target_window_field')
    ...
    state['records'].append({'completed': False, 'id': state['next_id'], 'initiative_field': arguments['initiative_field'], 'status_field': arguments['status_field'], 'target_window_field': arguments['target_window_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['initiative_field'] = arguments['initiative_field']
            record['status_field'] = arguments['status_field']
            record['target_window_field'] = arguments['target_window_field']
...
def control_0():
    ...
    _last_outcome = command_create({'initiative_field': _inputs['entry.initiative_field'].get(), 'status_field': _inputs['entry.status_field'].get(), 'target_window_field': _inputs['entry.target_window_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'id': selected_value('collection.primary', 'id'), 'initiative_field': _inputs['entry.initiative_field'].get(), 'status_field': _inputs['entry.status_field'].get(), 'target_window_field': _inputs['entry.target_window_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Initiative Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.initiative_field'] = Entry(_root, width=38)
    _inputs['entry.initiative_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Target Window Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.target_window_field'] = Entry(_root, width=38)
    _inputs['entry.target_window_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Status Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.status_field'] = Entry(_root, width=38)
    _inputs['entry.status_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}, 'id': 'product-roadmap.lifecycle', 'input': {'steps': [{'arguments': {'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}, 'inputs': {'entry.initiative_field': 'initiative_field-value', 'entry.primary': 'A', 'entry.status_field': 'status_field-value', 'entry.target_window_field': 'target_window_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'B'}]}}, 'inputs': {'entry.initiative_field': 'initiative_field-value', 'entry.primary': 'B', 'entry.status_field': 'status_field-value', 'entry.target_window_field': 'target_window_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'initiative_field': 'initiative_field-value', 'status_field': 'status_field-value', 'target_window_field': 'target_window_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
