...
APPLICATION_ID = 'uc://applications/editorial-calendar@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'content_stage_field', 'publish_at_field', 'editor_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/editorial-calendar/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_EDITORIAL_CALENDAR_STATE'
...
def command_create(arguments):
    ...
    if 'content_stage_field' not in arguments:
        return _failure('missing-content_stage_field')
    if 'publish_at_field' not in arguments:
        return _failure('missing-publish_at_field')
    if 'editor_field' not in arguments:
        return _failure('missing-editor_field')
    ...
    state['records'].append({'completed': False, 'content_stage_field': arguments['content_stage_field'], 'editor_field': arguments['editor_field'], 'id': state['next_id'], 'publish_at_field': arguments['publish_at_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['content_stage_field'] = arguments['content_stage_field']
            record['editor_field'] = arguments['editor_field']
            record['publish_at_field'] = arguments['publish_at_field']
...
def control_0():
    ...
    _last_outcome = command_create({'content_stage_field': _inputs['entry.content_stage_field'].get(), 'editor_field': _inputs['entry.editor_field'].get(), 'publish_at_field': _inputs['entry.publish_at_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'content_stage_field': _inputs['entry.content_stage_field'].get(), 'editor_field': _inputs['entry.editor_field'].get(), 'id': selected_value('collection.primary', 'id'), 'publish_at_field': _inputs['entry.publish_at_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Editorial calendar')
    ...
    Label(_root, text='Content Stage Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.content_stage_field'] = Entry(_root, width=38)
    _inputs['entry.content_stage_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Publish At Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.publish_at_field'] = Entry(_root, width=38)
    _inputs['entry.publish_at_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Editor Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.editor_field'] = Entry(_root, width=38)
    _inputs['entry.editor_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}, 'id': 'editorial-calendar.lifecycle', 'input': {'steps': [{'arguments': {'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'publish_at_field': 'publish_at_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}, 'inputs': {'entry.content_stage_field': 'content_stage_field-value', 'entry.editor_field': 'editor_field-value', 'entry.primary': 'A', 'entry.publish_at_field': 'publish_at_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'B'}]}}, 'inputs': {'entry.content_stage_field': 'content_stage_field-value', 'entry.editor_field': 'editor_field-value', 'entry.primary': 'B', 'entry.publish_at_field': 'publish_at_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'publish_at_field': 'publish_at_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'id': 1, 'publish_at_field': 'publish_at_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'publish_at_field': 'publish_at_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'content_stage_field': 'content_stage_field-value', 'editor_field': 'editor_field-value', 'publish_at_field': 'publish_at_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
