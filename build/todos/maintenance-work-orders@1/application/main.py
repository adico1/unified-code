...
APPLICATION_ID = 'uc://applications/maintenance-work-orders@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'asset_field', 'work_order_state_field', 'technician_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/maintenance-work-orders/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_MAINTENANCE_WORK_ORDERS_STATE'
...
def command_create(arguments):
    ...
    if 'asset_field' not in arguments:
        return _failure('missing-asset_field')
    if 'work_order_state_field' not in arguments:
        return _failure('missing-work_order_state_field')
    if 'technician_field' not in arguments:
        return _failure('missing-technician_field')
    ...
    state['records'].append({'asset_field': arguments['asset_field'], 'completed': False, 'id': state['next_id'], 'technician_field': arguments['technician_field'], 'title': arguments['title'], 'work_order_state_field': arguments['work_order_state_field']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['asset_field'] = arguments['asset_field']
            record['technician_field'] = arguments['technician_field']
            record['title'] = arguments['title']
            record['work_order_state_field'] = arguments['work_order_state_field']
    persist_state()
    present_state()
    return _success()
...
def control_0():
    ...
    _last_outcome = command_create({'asset_field': _inputs['entry.asset_field'].get(), 'technician_field': _inputs['entry.technician_field'].get(), 'title': _inputs['entry.primary'].get(), 'work_order_state_field': _inputs['entry.work_order_state_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'asset_field': _inputs['entry.asset_field'].get(), 'id': selected_value('collection.primary', 'id'), 'technician_field': _inputs['entry.technician_field'].get(), 'title': _inputs['entry.primary'].get(), 'work_order_state_field': _inputs['entry.work_order_state_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Maintenance work orders')
    ...
    Label(_root, text='Asset Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.asset_field'] = Entry(_root, width=38)
    _inputs['entry.asset_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Work Order State Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.work_order_state_field'] = Entry(_root, width=38)
    _inputs['entry.work_order_state_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Technician Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.technician_field'] = Entry(_root, width=38)
    _inputs['entry.technician_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': False, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': True, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': True, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': True, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}, 'id': 'maintenance-work-orders.lifecycle', 'input': {'steps': [{'arguments': {'asset_field': 'asset_field-value', 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': False, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': False, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}, 'inputs': {'entry.asset_field': 'asset_field-value', 'entry.primary': 'A', 'entry.technician_field': 'technician_field-value', 'entry.work_order_state_field': 'work_order_state_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': False, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'B', 'work_order_state_field': 'work_order_state_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': False, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'B', 'work_order_state_field': 'work_order_state_field-value'}]}}, 'inputs': {'entry.asset_field': 'asset_field-value', 'entry.primary': 'B', 'entry.technician_field': 'technician_field-value', 'entry.work_order_state_field': 'work_order_state_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'asset_field': 'asset_field-value', 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': True, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'asset_field': 'asset_field-value', 'completed': True, 'id': 1, 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'asset_field': 'asset_field-value', 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'asset_field': 'asset_field-value', 'technician_field': 'technician_field-value', 'title': 'A', 'work_order_state_field': 'work_order_state_field-value'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
