...
APPLICATION_ID = 'uc://applications/study-homework@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'course_field', 'due_at_field', 'study_state_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/study-homework/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_STUDY_HOMEWORK_STATE'
...
def command_create(arguments):
    ...
    if 'course_field' not in arguments:
        return _failure('missing-course_field')
    if 'due_at_field' not in arguments:
        return _failure('missing-due_at_field')
    if 'study_state_field' not in arguments:
        return _failure('missing-study_state_field')
    ...
    state['records'].append({'completed': False, 'course_field': arguments['course_field'], 'due_at_field': arguments['due_at_field'], 'id': state['next_id'], 'study_state_field': arguments['study_state_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['course_field'] = arguments['course_field']
            record['due_at_field'] = arguments['due_at_field']
            record['study_state_field'] = arguments['study_state_field']
...
def control_0():
    ...
    _last_outcome = command_create({'course_field': _inputs['entry.course_field'].get(), 'due_at_field': _inputs['entry.due_at_field'].get(), 'study_state_field': _inputs['entry.study_state_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'course_field': _inputs['entry.course_field'].get(), 'due_at_field': _inputs['entry.due_at_field'].get(), 'id': selected_value('collection.primary', 'id'), 'study_state_field': _inputs['entry.study_state_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Study and homework planner')
    ...
    Label(_root, text='Course Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.course_field'] = Entry(_root, width=38)
    _inputs['entry.course_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    ...
    Label(_root, text='Study State Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.study_state_field'] = Entry(_root, width=38)
    _inputs['entry.study_state_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}, 'id': 'study-homework.lifecycle', 'input': {'steps': [{'arguments': {'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'study_state_field': 'study_state_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}, 'inputs': {'entry.course_field': 'course_field-value', 'entry.due_at_field': 'due_at_field-value', 'entry.primary': 'A', 'entry.study_state_field': 'study_state_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'B'}]}}, 'inputs': {'entry.course_field': 'course_field-value', 'entry.due_at_field': 'due_at_field-value', 'entry.primary': 'B', 'entry.study_state_field': 'study_state_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'study_state_field': 'study_state_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'id': 1, 'study_state_field': 'study_state_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'study_state_field': 'study_state_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'course_field': 'course_field-value', 'due_at_field': 'due_at_field-value', 'study_state_field': 'study_state_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
