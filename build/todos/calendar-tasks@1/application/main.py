...
APPLICATION_ID = 'uc://applications/calendar-tasks@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'start_at_field', 'due_at_field', 'calendar_placement_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/calendar-tasks/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_CALENDAR_TASKS_STATE'
...
def command_create(arguments):
    ...
    if 'start_at_field' not in arguments:
        return _failure('missing-start_at_field')
    if 'due_at_field' not in arguments:
        return _failure('missing-due_at_field')
    if 'calendar_placement_field' not in arguments:
        return _failure('missing-calendar_placement_field')
    ...
    state['records'].append({'calendar_placement_field': arguments['calendar_placement_field'], 'completed': False, 'due_at_field': arguments['due_at_field'], 'id': state['next_id'], 'start_at_field': arguments['start_at_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['calendar_placement_field'] = arguments['calendar_placement_field']
            record['due_at_field'] = arguments['due_at_field']
            record['start_at_field'] = arguments['start_at_field']
...
def control_0():
    ...
    _last_outcome = command_create({'calendar_placement_field': _inputs['entry.calendar_placement_field'].get(), 'due_at_field': _inputs['entry.due_at_field'].get(), 'start_at_field': _inputs['entry.start_at_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'calendar_placement_field': _inputs['entry.calendar_placement_field'].get(), 'due_at_field': _inputs['entry.due_at_field'].get(), 'id': selected_value('collection.primary', 'id'), 'start_at_field': _inputs['entry.start_at_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Start At Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.start_at_field'] = Entry(_root, width=38)
    _inputs['entry.start_at_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Due At Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.due_at_field'] = Entry(_root, width=38)
    _inputs['entry.due_at_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Calendar Placement Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.calendar_placement_field'] = Entry(_root, width=38)
    _inputs['entry.calendar_placement_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': False, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': True, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': True, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': True, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}, 'id': 'calendar-tasks.lifecycle', 'input': {'steps': [{'arguments': {'calendar_placement_field': 'calendar_placement_field-value', 'due_at_field': 'due_at_field-value', 'start_at_field': 'start_at_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': False, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': False, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}, 'inputs': {'entry.calendar_placement_field': 'calendar_placement_field-value', 'entry.due_at_field': 'due_at_field-value', 'entry.primary': 'A', 'entry.start_at_field': 'start_at_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': False, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': False, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'B'}]}}, 'inputs': {'entry.calendar_placement_field': 'calendar_placement_field-value', 'entry.due_at_field': 'due_at_field-value', 'entry.primary': 'B', 'entry.start_at_field': 'start_at_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'calendar_placement_field': 'calendar_placement_field-value', 'due_at_field': 'due_at_field-value', 'start_at_field': 'start_at_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': True, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'calendar_placement_field': 'calendar_placement_field-value', 'completed': True, 'due_at_field': 'due_at_field-value', 'id': 1, 'start_at_field': 'start_at_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'calendar_placement_field': 'calendar_placement_field-value', 'due_at_field': 'due_at_field-value', 'start_at_field': 'start_at_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'calendar_placement_field': 'calendar_placement_field-value', 'due_at_field': 'due_at_field-value', 'start_at_field': 'start_at_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
