...
APPLICATION_ID = 'uc://applications/approvals@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'reviewer_field', 'approval_state_field', 'decision_evidence_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/approvals/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_APPROVALS_STATE'
...
def command_create(arguments):
    ...
    if 'reviewer_field' not in arguments:
        return _failure('missing-reviewer_field')
    if 'approval_state_field' not in arguments:
        return _failure('missing-approval_state_field')
    if 'decision_evidence_field' not in arguments:
        return _failure('missing-decision_evidence_field')
    ...
    state['records'].append({'approval_state_field': arguments['approval_state_field'], 'completed': False, 'decision_evidence_field': arguments['decision_evidence_field'], 'id': state['next_id'], 'reviewer_field': arguments['reviewer_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['approval_state_field'] = arguments['approval_state_field']
            record['decision_evidence_field'] = arguments['decision_evidence_field']
            record['reviewer_field'] = arguments['reviewer_field']
...
def control_0():
    ...
    _last_outcome = command_create({'approval_state_field': _inputs['entry.approval_state_field'].get(), 'decision_evidence_field': _inputs['entry.decision_evidence_field'].get(), 'reviewer_field': _inputs['entry.reviewer_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'approval_state_field': _inputs['entry.approval_state_field'].get(), 'decision_evidence_field': _inputs['entry.decision_evidence_field'].get(), 'id': selected_value('collection.primary', 'id'), 'reviewer_field': _inputs['entry.reviewer_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Reviewer Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.reviewer_field'] = Entry(_root, width=38)
    _inputs['entry.reviewer_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Approval State Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.approval_state_field'] = Entry(_root, width=38)
    _inputs['entry.approval_state_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Decision Evidence Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.decision_evidence_field'] = Entry(_root, width=38)
    _inputs['entry.decision_evidence_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': False, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': True, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': True, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': True, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}, 'id': 'approvals.lifecycle', 'input': {'steps': [{'arguments': {'approval_state_field': 'approval_state_field-value', 'decision_evidence_field': 'decision_evidence_field-value', 'reviewer_field': 'reviewer_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': False, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': False, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}, 'inputs': {'entry.approval_state_field': 'approval_state_field-value', 'entry.decision_evidence_field': 'decision_evidence_field-value', 'entry.primary': 'A', 'entry.reviewer_field': 'reviewer_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': False, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': False, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'B'}]}}, 'inputs': {'entry.approval_state_field': 'approval_state_field-value', 'entry.decision_evidence_field': 'decision_evidence_field-value', 'entry.primary': 'B', 'entry.reviewer_field': 'reviewer_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'approval_state_field': 'approval_state_field-value', 'decision_evidence_field': 'decision_evidence_field-value', 'reviewer_field': 'reviewer_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': True, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'approval_state_field': 'approval_state_field-value', 'completed': True, 'decision_evidence_field': 'decision_evidence_field-value', 'id': 1, 'reviewer_field': 'reviewer_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'approval_state_field': 'approval_state_field-value', 'decision_evidence_field': 'decision_evidence_field-value', 'reviewer_field': 'reviewer_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'approval_state_field': 'approval_state_field-value', 'decision_evidence_field': 'decision_evidence_field-value', 'reviewer_field': 'reviewer_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
