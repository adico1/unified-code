...
APPLICATION_ID = 'uc://applications/collaboration-sync-plan@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'shared_state_field', 'conflict_policy_field', 'synchronization_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/collaboration-sync-plan/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_COLLABORATION_SYNC_PLAN_STATE'
...
def command_create(arguments):
    ...
    if 'shared_state_field' not in arguments:
        return _failure('missing-shared_state_field')
    if 'conflict_policy_field' not in arguments:
        return _failure('missing-conflict_policy_field')
    if 'synchronization_field' not in arguments:
        return _failure('missing-synchronization_field')
    ...
    state['records'].append({'completed': False, 'conflict_policy_field': arguments['conflict_policy_field'], 'id': state['next_id'], 'shared_state_field': arguments['shared_state_field'], 'synchronization_field': arguments['synchronization_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['conflict_policy_field'] = arguments['conflict_policy_field']
            record['shared_state_field'] = arguments['shared_state_field']
            record['synchronization_field'] = arguments['synchronization_field']
...
def control_0():
    ...
    _last_outcome = command_create({'conflict_policy_field': _inputs['entry.conflict_policy_field'].get(), 'shared_state_field': _inputs['entry.shared_state_field'].get(), 'synchronization_field': _inputs['entry.synchronization_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'conflict_policy_field': _inputs['entry.conflict_policy_field'].get(), 'id': selected_value('collection.primary', 'id'), 'shared_state_field': _inputs['entry.shared_state_field'].get(), 'synchronization_field': _inputs['entry.synchronization_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Collaboration and sync planning record manager')
    ...
    Label(_root, text='Shared State Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.shared_state_field'] = Entry(_root, width=38)
    _inputs['entry.shared_state_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Conflict Policy Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.conflict_policy_field'] = Entry(_root, width=38)
    _inputs['entry.conflict_policy_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Synchronization Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.synchronization_field'] = Entry(_root, width=38)
    _inputs['entry.synchronization_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}, 'id': 'collaboration-sync-plan.lifecycle', 'input': {'steps': [{'arguments': {'conflict_policy_field': 'conflict_policy_field-value', 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}, 'inputs': {'entry.conflict_policy_field': 'conflict_policy_field-value', 'entry.primary': 'A', 'entry.shared_state_field': 'shared_state_field-value', 'entry.synchronization_field': 'synchronization_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'B'}]}}, 'inputs': {'entry.conflict_policy_field': 'conflict_policy_field-value', 'entry.primary': 'B', 'entry.shared_state_field': 'shared_state_field-value', 'entry.synchronization_field': 'synchronization_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'conflict_policy_field': 'conflict_policy_field-value', 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'conflict_policy_field': 'conflict_policy_field-value', 'id': 1, 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'conflict_policy_field': 'conflict_policy_field-value', 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'conflict_policy_field': 'conflict_policy_field-value', 'shared_state_field': 'shared_state_field-value', 'synchronization_field': 'synchronization_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
