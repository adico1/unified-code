...
APPLICATION_ID = 'uc://applications/dependency-plan@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'dependency_edge_field', 'blocked_state_field', 'critical_order_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/dependency-plan/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_DEPENDENCY_PLAN_STATE'
...
def command_create(arguments):
    ...
    if 'dependency_edge_field' not in arguments:
        return _failure('missing-dependency_edge_field')
    if 'blocked_state_field' not in arguments:
        return _failure('missing-blocked_state_field')
    if 'critical_order_field' not in arguments:
        return _failure('missing-critical_order_field')
    ...
    state['records'].append({'blocked_state_field': arguments['blocked_state_field'], 'completed': False, 'critical_order_field': arguments['critical_order_field'], 'dependency_edge_field': arguments['dependency_edge_field'], 'id': state['next_id'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['blocked_state_field'] = arguments['blocked_state_field']
            record['critical_order_field'] = arguments['critical_order_field']
            record['dependency_edge_field'] = arguments['dependency_edge_field']
...
def control_0():
    ...
    _last_outcome = command_create({'blocked_state_field': _inputs['entry.blocked_state_field'].get(), 'critical_order_field': _inputs['entry.critical_order_field'].get(), 'dependency_edge_field': _inputs['entry.dependency_edge_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'blocked_state_field': _inputs['entry.blocked_state_field'].get(), 'critical_order_field': _inputs['entry.critical_order_field'].get(), 'dependency_edge_field': _inputs['entry.dependency_edge_field'].get(), 'id': selected_value('collection.primary', 'id'), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Dependency planning record manager')
    ...
    Label(_root, text='Dependency Edge Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.dependency_edge_field'] = Entry(_root, width=38)
    _inputs['entry.dependency_edge_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Blocked State Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.blocked_state_field'] = Entry(_root, width=38)
    _inputs['entry.blocked_state_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Critical Order Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.critical_order_field'] = Entry(_root, width=38)
    _inputs['entry.critical_order_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'blocked_state_field': 'blocked_state_field-value', 'completed': False, 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'id': 1, 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'blocked_state_field': 'blocked_state_field-value', 'completed': False, 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'id': 1, 'title': 'A'}]}}, 'inputs': {'entry.blocked_state_field': 'blocked_state_field-value', 'entry.critical_order_field': 'critical_order_field-value', 'entry.dependency_edge_field': 'dependency_edge_field-value', 'entry.primary': 'A'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'blocked_state_field': 'blocked_state_field-value', 'completed': False, 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'id': 1, 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'blocked_state_field': 'blocked_state_field-value', 'completed': False, 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'id': 1, 'title': 'B'}]}}, 'inputs': {'entry.blocked_state_field': 'blocked_state_field-value', 'entry.critical_order_field': 'critical_order_field-value', 'entry.dependency_edge_field': 'dependency_edge_field-value', 'entry.primary': 'B'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'blocked_state_field': 'blocked_state_field-value', 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'blocked_state_field': 'blocked_state_field-value', 'completed': True, 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'id': 1, 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'blocked_state_field': 'blocked_state_field-value', 'completed': True, 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'id': 1, 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'blocked_state_field': 'blocked_state_field-value', 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'blocked_state_field': 'blocked_state_field-value', 'critical_order_field': 'critical_order_field-value', 'dependency_edge_field': 'dependency_edge_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
