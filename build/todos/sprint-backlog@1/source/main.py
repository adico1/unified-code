...
APPLICATION_ID = 'uc://applications/sprint-backlog@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'estimate_field', 'sprint_field', 'workflow_state_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/sprint-backlog/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_SPRINT_BACKLOG_STATE'
...
def command_create(arguments):
    ...
    if 'estimate_field' not in arguments:
        return _failure('missing-estimate_field')
    if 'sprint_field' not in arguments:
        return _failure('missing-sprint_field')
    if 'workflow_state_field' not in arguments:
        return _failure('missing-workflow_state_field')
    ...
    state['records'].append({'completed': False, 'estimate_field': arguments['estimate_field'], 'id': state['next_id'], 'sprint_field': arguments['sprint_field'], 'title': arguments['title'], 'workflow_state_field': arguments['workflow_state_field']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['estimate_field'] = arguments['estimate_field']
            record['sprint_field'] = arguments['sprint_field']
            record['title'] = arguments['title']
            record['workflow_state_field'] = arguments['workflow_state_field']
    persist_state()
    present_state()
    return _success()
...
def control_0():
    ...
    _last_outcome = command_create({'estimate_field': _inputs['entry.estimate_field'].get(), 'sprint_field': _inputs['entry.sprint_field'].get(), 'title': _inputs['entry.primary'].get(), 'workflow_state_field': _inputs['entry.workflow_state_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'estimate_field': _inputs['entry.estimate_field'].get(), 'id': selected_value('collection.primary', 'id'), 'sprint_field': _inputs['entry.sprint_field'].get(), 'title': _inputs['entry.primary'].get(), 'workflow_state_field': _inputs['entry.workflow_state_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Estimate Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.estimate_field'] = Entry(_root, width=38)
    _inputs['entry.estimate_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Sprint Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.sprint_field'] = Entry(_root, width=38)
    _inputs['entry.sprint_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Workflow State Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.workflow_state_field'] = Entry(_root, width=38)
    _inputs['entry.workflow_state_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}, 'id': 'sprint-backlog.lifecycle', 'input': {'steps': [{'arguments': {'estimate_field': 'estimate_field-value', 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}, 'inputs': {'entry.estimate_field': 'estimate_field-value', 'entry.primary': 'A', 'entry.sprint_field': 'sprint_field-value', 'entry.workflow_state_field': 'workflow_state_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'B', 'workflow_state_field': 'workflow_state_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'B', 'workflow_state_field': 'workflow_state_field-value'}]}}, 'inputs': {'entry.estimate_field': 'estimate_field-value', 'entry.primary': 'B', 'entry.sprint_field': 'sprint_field-value', 'entry.workflow_state_field': 'workflow_state_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'estimate_field': 'estimate_field-value', 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'estimate_field': 'estimate_field-value', 'id': 1, 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'estimate_field': 'estimate_field-value', 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'estimate_field': 'estimate_field-value', 'sprint_field': 'sprint_field-value', 'title': 'A', 'workflow_state_field': 'workflow_state_field-value'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
