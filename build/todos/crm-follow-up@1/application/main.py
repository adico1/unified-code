...
APPLICATION_ID = 'uc://applications/crm-follow-up@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'contact_field', 'follow_up_at_field', 'relationship_stage_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/crm-follow-up/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_CRM_FOLLOW_UP_STATE'
...
def command_create(arguments):
    ...
    if 'contact_field' not in arguments:
        return _failure('missing-contact_field')
    if 'follow_up_at_field' not in arguments:
        return _failure('missing-follow_up_at_field')
    if 'relationship_stage_field' not in arguments:
        return _failure('missing-relationship_stage_field')
    ...
    state['records'].append({'completed': False, 'contact_field': arguments['contact_field'], 'follow_up_at_field': arguments['follow_up_at_field'], 'id': state['next_id'], 'relationship_stage_field': arguments['relationship_stage_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['contact_field'] = arguments['contact_field']
            record['follow_up_at_field'] = arguments['follow_up_at_field']
            record['relationship_stage_field'] = arguments['relationship_stage_field']
...
def control_0():
    ...
    _last_outcome = command_create({'contact_field': _inputs['entry.contact_field'].get(), 'follow_up_at_field': _inputs['entry.follow_up_at_field'].get(), 'relationship_stage_field': _inputs['entry.relationship_stage_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'contact_field': _inputs['entry.contact_field'].get(), 'follow_up_at_field': _inputs['entry.follow_up_at_field'].get(), 'id': selected_value('collection.primary', 'id'), 'relationship_stage_field': _inputs['entry.relationship_stage_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('CRM follow-up list')
    ...
    Label(_root, text='Contact Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.contact_field'] = Entry(_root, width=38)
    _inputs['entry.contact_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Follow Up At Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.follow_up_at_field'] = Entry(_root, width=38)
    _inputs['entry.follow_up_at_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Relationship Stage Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.relationship_stage_field'] = Entry(_root, width=38)
    _inputs['entry.relationship_stage_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}, 'id': 'crm-follow-up.lifecycle', 'input': {'steps': [{'arguments': {'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}, 'inputs': {'entry.contact_field': 'contact_field-value', 'entry.follow_up_at_field': 'follow_up_at_field-value', 'entry.primary': 'A', 'entry.relationship_stage_field': 'relationship_stage_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'B'}]}}, 'inputs': {'entry.contact_field': 'contact_field-value', 'entry.follow_up_at_field': 'follow_up_at_field-value', 'entry.primary': 'B', 'entry.relationship_stage_field': 'relationship_stage_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'id': 1, 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'contact_field': 'contact_field-value', 'follow_up_at_field': 'follow_up_at_field-value', 'relationship_stage_field': 'relationship_stage_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
