...
APPLICATION_ID = 'uc://applications/offline-security-plan@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'local_storage_field', 'encryption_boundary_field', 'offline_operation_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/offline-security-plan/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_OFFLINE_SECURITY_PLAN_STATE'
...
def command_create(arguments):
    ...
    if 'local_storage_field' not in arguments:
        return _failure('missing-local_storage_field')
    if 'encryption_boundary_field' not in arguments:
        return _failure('missing-encryption_boundary_field')
    if 'offline_operation_field' not in arguments:
        return _failure('missing-offline_operation_field')
    ...
    state['records'].append({'completed': False, 'encryption_boundary_field': arguments['encryption_boundary_field'], 'id': state['next_id'], 'local_storage_field': arguments['local_storage_field'], 'offline_operation_field': arguments['offline_operation_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['encryption_boundary_field'] = arguments['encryption_boundary_field']
            record['local_storage_field'] = arguments['local_storage_field']
            record['offline_operation_field'] = arguments['offline_operation_field']
...
def control_0():
    ...
    _last_outcome = command_create({'encryption_boundary_field': _inputs['entry.encryption_boundary_field'].get(), 'local_storage_field': _inputs['entry.local_storage_field'].get(), 'offline_operation_field': _inputs['entry.offline_operation_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'encryption_boundary_field': _inputs['entry.encryption_boundary_field'].get(), 'id': selected_value('collection.primary', 'id'), 'local_storage_field': _inputs['entry.local_storage_field'].get(), 'offline_operation_field': _inputs['entry.offline_operation_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Offline security planning record manager')
    ...
    Label(_root, text='Local Storage Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.local_storage_field'] = Entry(_root, width=38)
    _inputs['entry.local_storage_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Encryption Boundary Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.encryption_boundary_field'] = Entry(_root, width=38)
    _inputs['entry.encryption_boundary_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Offline Operation Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.offline_operation_field'] = Entry(_root, width=38)
    _inputs['entry.offline_operation_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}, 'id': 'offline-security-plan.lifecycle', 'input': {'steps': [{'arguments': {'encryption_boundary_field': 'encryption_boundary_field-value', 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}, 'inputs': {'entry.encryption_boundary_field': 'encryption_boundary_field-value', 'entry.local_storage_field': 'local_storage_field-value', 'entry.offline_operation_field': 'offline_operation_field-value', 'entry.primary': 'A'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'B'}]}}, 'inputs': {'entry.encryption_boundary_field': 'encryption_boundary_field-value', 'entry.local_storage_field': 'local_storage_field-value', 'entry.offline_operation_field': 'offline_operation_field-value', 'entry.primary': 'B'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'encryption_boundary_field': 'encryption_boundary_field-value', 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'encryption_boundary_field': 'encryption_boundary_field-value', 'id': 1, 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'encryption_boundary_field': 'encryption_boundary_field-value', 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'encryption_boundary_field': 'encryption_boundary_field-value', 'local_storage_field': 'local_storage_field-value', 'offline_operation_field': 'offline_operation_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
