...
APPLICATION_ID = 'uc://applications/issue-tracker@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'status_field', 'labels_field', 'assignee_field', 'discussion_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/issue-tracker/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_ISSUE_TRACKER_STATE'
...
def command_create(arguments):
    ...
    if 'status_field' not in arguments:
        return _failure('missing-status_field')
    if 'labels_field' not in arguments:
        return _failure('missing-labels_field')
    if 'assignee_field' not in arguments:
        return _failure('missing-assignee_field')
    if 'discussion_field' not in arguments:
        return _failure('missing-discussion_field')
    ...
    state['records'].append({'assignee_field': arguments['assignee_field'], 'completed': False, 'discussion_field': arguments['discussion_field'], 'id': state['next_id'], 'labels_field': arguments['labels_field'], 'status_field': arguments['status_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['assignee_field'] = arguments['assignee_field']
            record['discussion_field'] = arguments['discussion_field']
            record['labels_field'] = arguments['labels_field']
            record['status_field'] = arguments['status_field']
...
def control_0():
    ...
    _last_outcome = command_create({'assignee_field': _inputs['entry.assignee_field'].get(), 'discussion_field': _inputs['entry.discussion_field'].get(), 'labels_field': _inputs['entry.labels_field'].get(), 'status_field': _inputs['entry.status_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'assignee_field': _inputs['entry.assignee_field'].get(), 'discussion_field': _inputs['entry.discussion_field'].get(), 'id': selected_value('collection.primary', 'id'), 'labels_field': _inputs['entry.labels_field'].get(), 'status_field': _inputs['entry.status_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Status Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.status_field'] = Entry(_root, width=38)
    _inputs['entry.status_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Labels Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.labels_field'] = Entry(_root, width=38)
    _inputs['entry.labels_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Assignee Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.assignee_field'] = Entry(_root, width=38)
    _inputs['entry.assignee_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Discussion Field').grid(row=4, column=0, sticky='w')
    _inputs['entry.discussion_field'] = Entry(_root, width=38)
    _inputs['entry.discussion_field'].grid(row=4, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}, 'id': 'issue-tracker.lifecycle', 'input': {'steps': [{'arguments': {'assignee_field': 'assignee_field-value', 'discussion_field': 'discussion_field-value', 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}, 'inputs': {'entry.assignee_field': 'assignee_field-value', 'entry.discussion_field': 'discussion_field-value', 'entry.labels_field': 'labels_field-value', 'entry.primary': 'A', 'entry.status_field': 'status_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'B'}]}}, 'inputs': {'entry.assignee_field': 'assignee_field-value', 'entry.discussion_field': 'discussion_field-value', 'entry.labels_field': 'labels_field-value', 'entry.primary': 'B', 'entry.status_field': 'status_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'discussion_field': 'discussion_field-value', 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'discussion_field': 'discussion_field-value', 'id': 1, 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'discussion_field': 'discussion_field-value', 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'discussion_field': 'discussion_field-value', 'labels_field': 'labels_field-value', 'status_field': 'status_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
