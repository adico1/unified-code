...
APPLICATION_ID = 'uc://applications/bug-tracker@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'severity_field', 'reproduction_field', 'resolution_state_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/bug-tracker/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_BUG_TRACKER_STATE'
...
def command_create(arguments):
    ...
    if 'severity_field' not in arguments:
        return _failure('missing-severity_field')
    if 'reproduction_field' not in arguments:
        return _failure('missing-reproduction_field')
    if 'resolution_state_field' not in arguments:
        return _failure('missing-resolution_state_field')
    ...
    state['records'].append({'completed': False, 'id': state['next_id'], 'reproduction_field': arguments['reproduction_field'], 'resolution_state_field': arguments['resolution_state_field'], 'severity_field': arguments['severity_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['reproduction_field'] = arguments['reproduction_field']
            record['resolution_state_field'] = arguments['resolution_state_field']
            record['severity_field'] = arguments['severity_field']
...
def control_0():
    ...
    _last_outcome = command_create({'reproduction_field': _inputs['entry.reproduction_field'].get(), 'resolution_state_field': _inputs['entry.resolution_state_field'].get(), 'severity_field': _inputs['entry.severity_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'id': selected_value('collection.primary', 'id'), 'reproduction_field': _inputs['entry.reproduction_field'].get(), 'resolution_state_field': _inputs['entry.resolution_state_field'].get(), 'severity_field': _inputs['entry.severity_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Severity Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.severity_field'] = Entry(_root, width=38)
    _inputs['entry.severity_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Reproduction Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.reproduction_field'] = Entry(_root, width=38)
    _inputs['entry.reproduction_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Resolution State Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.resolution_state_field'] = Entry(_root, width=38)
    _inputs['entry.resolution_state_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}, 'id': 'bug-tracker.lifecycle', 'input': {'steps': [{'arguments': {'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}, 'inputs': {'entry.primary': 'A', 'entry.reproduction_field': 'reproduction_field-value', 'entry.resolution_state_field': 'resolution_state_field-value', 'entry.severity_field': 'severity_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'B'}]}}, 'inputs': {'entry.primary': 'B', 'entry.reproduction_field': 'reproduction_field-value', 'entry.resolution_state_field': 'resolution_state_field-value', 'entry.severity_field': 'severity_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'reproduction_field': 'reproduction_field-value', 'resolution_state_field': 'resolution_state_field-value', 'severity_field': 'severity_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
