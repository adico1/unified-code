...
APPLICATION_ID = 'uc://applications/team-assignments@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'assignee_field', 'ownership_field', 'handoff_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/team-assignments/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_TEAM_ASSIGNMENTS_STATE'
...
def command_create(arguments):
    ...
    if 'ownership_field' not in arguments:
        return _failure('missing-ownership_field')
    if 'handoff_field' not in arguments:
        return _failure('missing-handoff_field')
    ...
    state['records'].append({'assignee_field': arguments['assignee_field'], 'completed': False, 'handoff_field': arguments['handoff_field'], 'id': state['next_id'], 'ownership_field': arguments['ownership_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            ...
            record['handoff_field'] = arguments['handoff_field']
            record['ownership_field'] = arguments['ownership_field']
...
def control_0():
    ...
    _last_outcome = command_create({'assignee_field': _inputs['entry.assignee_field'].get(), 'handoff_field': _inputs['entry.handoff_field'].get(), 'ownership_field': _inputs['entry.ownership_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'assignee_field': _inputs['entry.assignee_field'].get(), 'handoff_field': _inputs['entry.handoff_field'].get(), 'id': selected_value('collection.primary', 'id'), 'ownership_field': _inputs['entry.ownership_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Assignee Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.assignee_field'] = Entry(_root, width=38)
    _inputs['entry.assignee_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Ownership Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.ownership_field'] = Entry(_root, width=38)
    _inputs['entry.ownership_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Handoff Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.handoff_field'] = Entry(_root, width=38)
    _inputs['entry.handoff_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}, 'id': 'team-assignments.lifecycle', 'input': {'steps': [{'arguments': {'assignee_field': 'assignee_field-value', 'handoff_field': 'handoff_field-value', 'ownership_field': 'ownership_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}, 'inputs': {'entry.assignee_field': 'assignee_field-value', 'entry.handoff_field': 'handoff_field-value', 'entry.ownership_field': 'ownership_field-value', 'entry.primary': 'A'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'B'}]}}, 'inputs': {'entry.assignee_field': 'assignee_field-value', 'entry.handoff_field': 'handoff_field-value', 'entry.ownership_field': 'ownership_field-value', 'entry.primary': 'B'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'handoff_field': 'handoff_field-value', 'ownership_field': 'ownership_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'handoff_field': 'handoff_field-value', 'id': 1, 'ownership_field': 'ownership_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'handoff_field': 'handoff_field-value', 'ownership_field': 'ownership_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'handoff_field': 'handoff_field-value', 'ownership_field': 'ownership_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
