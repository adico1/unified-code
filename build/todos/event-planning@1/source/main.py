...
APPLICATION_ID = 'uc://applications/event-planning@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'event_phase_field', 'owner_field', 'deadline_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/event-planning/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_EVENT_PLANNING_STATE'
...
def command_create(arguments):
    ...
    if 'event_phase_field' not in arguments:
        return _failure('missing-event_phase_field')
    if 'owner_field' not in arguments:
        return _failure('missing-owner_field')
    if 'deadline_field' not in arguments:
        return _failure('missing-deadline_field')
    ...
    state['records'].append({'completed': False, 'deadline_field': arguments['deadline_field'], 'event_phase_field': arguments['event_phase_field'], 'id': state['next_id'], 'owner_field': arguments['owner_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['deadline_field'] = arguments['deadline_field']
            record['event_phase_field'] = arguments['event_phase_field']
            record['owner_field'] = arguments['owner_field']
...
def control_0():
    ...
    _last_outcome = command_create({'deadline_field': _inputs['entry.deadline_field'].get(), 'event_phase_field': _inputs['entry.event_phase_field'].get(), 'owner_field': _inputs['entry.owner_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'deadline_field': _inputs['entry.deadline_field'].get(), 'event_phase_field': _inputs['entry.event_phase_field'].get(), 'id': selected_value('collection.primary', 'id'), 'owner_field': _inputs['entry.owner_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Event planning checklist')
    ...
    Label(_root, text='Event Phase Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.event_phase_field'] = Entry(_root, width=38)
    _inputs['entry.event_phase_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Owner Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.owner_field'] = Entry(_root, width=38)
    _inputs['entry.owner_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Deadline Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.deadline_field'] = Entry(_root, width=38)
    _inputs['entry.deadline_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}, 'id': 'event-planning.lifecycle', 'input': {'steps': [{'arguments': {'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'owner_field': 'owner_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}, 'inputs': {'entry.deadline_field': 'deadline_field-value', 'entry.event_phase_field': 'event_phase_field-value', 'entry.owner_field': 'owner_field-value', 'entry.primary': 'A'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'B'}]}}, 'inputs': {'entry.deadline_field': 'deadline_field-value', 'entry.event_phase_field': 'event_phase_field-value', 'entry.owner_field': 'owner_field-value', 'entry.primary': 'B'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'owner_field': 'owner_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'id': 1, 'owner_field': 'owner_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'owner_field': 'owner_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'deadline_field': 'deadline_field-value', 'event_phase_field': 'event_phase_field-value', 'owner_field': 'owner_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
