...
APPLICATION_ID = 'uc://applications/research-pipeline@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'question_field', 'evidence_state_field', 'review_stage_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/research-pipeline/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_RESEARCH_PIPELINE_STATE'
...
def command_create(arguments):
    ...
    if 'question_field' not in arguments:
        return _failure('missing-question_field')
    if 'evidence_state_field' not in arguments:
        return _failure('missing-evidence_state_field')
    if 'review_stage_field' not in arguments:
        return _failure('missing-review_stage_field')
    ...
    state['records'].append({'completed': False, 'evidence_state_field': arguments['evidence_state_field'], 'id': state['next_id'], 'question_field': arguments['question_field'], 'review_stage_field': arguments['review_stage_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['evidence_state_field'] = arguments['evidence_state_field']
            record['question_field'] = arguments['question_field']
            record['review_stage_field'] = arguments['review_stage_field']
...
def control_0():
    ...
    _last_outcome = command_create({'evidence_state_field': _inputs['entry.evidence_state_field'].get(), 'question_field': _inputs['entry.question_field'].get(), 'review_stage_field': _inputs['entry.review_stage_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'evidence_state_field': _inputs['entry.evidence_state_field'].get(), 'id': selected_value('collection.primary', 'id'), 'question_field': _inputs['entry.question_field'].get(), 'review_stage_field': _inputs['entry.review_stage_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Question Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.question_field'] = Entry(_root, width=38)
    _inputs['entry.question_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Evidence State Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.evidence_state_field'] = Entry(_root, width=38)
    _inputs['entry.evidence_state_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Review Stage Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.review_stage_field'] = Entry(_root, width=38)
    _inputs['entry.review_stage_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}, 'id': 'research-pipeline.lifecycle', 'input': {'steps': [{'arguments': {'evidence_state_field': 'evidence_state_field-value', 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}, 'inputs': {'entry.evidence_state_field': 'evidence_state_field-value', 'entry.primary': 'A', 'entry.question_field': 'question_field-value', 'entry.review_stage_field': 'review_stage_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'B'}]}}, 'inputs': {'entry.evidence_state_field': 'evidence_state_field-value', 'entry.primary': 'B', 'entry.question_field': 'question_field-value', 'entry.review_stage_field': 'review_stage_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'evidence_state_field': 'evidence_state_field-value', 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'evidence_state_field': 'evidence_state_field-value', 'id': 1, 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'evidence_state_field': 'evidence_state_field-value', 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'evidence_state_field': 'evidence_state_field-value', 'question_field': 'question_field-value', 'review_stage_field': 'review_stage_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
