...
APPLICATION_ID = 'uc://applications/care-plan@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'care_action_field', 'schedule_field', 'completion_evidence_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/care-plan/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_CARE_PLAN_STATE'
...
def command_create(arguments):
    ...
    if 'care_action_field' not in arguments:
        return _failure('missing-care_action_field')
    if 'schedule_field' not in arguments:
        return _failure('missing-schedule_field')
    if 'completion_evidence_field' not in arguments:
        return _failure('missing-completion_evidence_field')
    ...
    state['records'].append({'care_action_field': arguments['care_action_field'], 'completed': False, 'completion_evidence_field': arguments['completion_evidence_field'], 'id': state['next_id'], 'schedule_field': arguments['schedule_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['care_action_field'] = arguments['care_action_field']
            record['completion_evidence_field'] = arguments['completion_evidence_field']
            record['schedule_field'] = arguments['schedule_field']
...
def control_0():
    ...
    _last_outcome = command_create({'care_action_field': _inputs['entry.care_action_field'].get(), 'completion_evidence_field': _inputs['entry.completion_evidence_field'].get(), 'schedule_field': _inputs['entry.schedule_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'care_action_field': _inputs['entry.care_action_field'].get(), 'completion_evidence_field': _inputs['entry.completion_evidence_field'].get(), 'id': selected_value('collection.primary', 'id'), 'schedule_field': _inputs['entry.schedule_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Care Action Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.care_action_field'] = Entry(_root, width=38)
    _inputs['entry.care_action_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Schedule Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.schedule_field'] = Entry(_root, width=38)
    _inputs['entry.schedule_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Completion Evidence Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.completion_evidence_field'] = Entry(_root, width=38)
    _inputs['entry.completion_evidence_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': False, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': True, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': True, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': True, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}, 'id': 'care-plan.lifecycle', 'input': {'steps': [{'arguments': {'care_action_field': 'care_action_field-value', 'completion_evidence_field': 'completion_evidence_field-value', 'schedule_field': 'schedule_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': False, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': False, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}, 'inputs': {'entry.care_action_field': 'care_action_field-value', 'entry.completion_evidence_field': 'completion_evidence_field-value', 'entry.primary': 'A', 'entry.schedule_field': 'schedule_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': False, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': False, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'B'}]}}, 'inputs': {'entry.care_action_field': 'care_action_field-value', 'entry.completion_evidence_field': 'completion_evidence_field-value', 'entry.primary': 'B', 'entry.schedule_field': 'schedule_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'care_action_field': 'care_action_field-value', 'completion_evidence_field': 'completion_evidence_field-value', 'schedule_field': 'schedule_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': True, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'care_action_field': 'care_action_field-value', 'completed': True, 'completion_evidence_field': 'completion_evidence_field-value', 'id': 1, 'schedule_field': 'schedule_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'care_action_field': 'care_action_field-value', 'completion_evidence_field': 'completion_evidence_field-value', 'schedule_field': 'schedule_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'care_action_field': 'care_action_field-value', 'completion_evidence_field': 'completion_evidence_field-value', 'schedule_field': 'schedule_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
