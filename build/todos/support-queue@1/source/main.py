...
APPLICATION_ID = 'uc://applications/support-queue@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'priority_field', 'service_state_field', 'response_deadline_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/support-queue/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_SUPPORT_QUEUE_STATE'
...
def command_create(arguments):
    ...
    if 'priority_field' not in arguments:
        return _failure('missing-priority_field')
    if 'service_state_field' not in arguments:
        return _failure('missing-service_state_field')
    if 'response_deadline_field' not in arguments:
        return _failure('missing-response_deadline_field')
    ...
    state['records'].append({'completed': False, 'id': state['next_id'], 'priority_field': arguments['priority_field'], 'response_deadline_field': arguments['response_deadline_field'], 'service_state_field': arguments['service_state_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['priority_field'] = arguments['priority_field']
            record['response_deadline_field'] = arguments['response_deadline_field']
            record['service_state_field'] = arguments['service_state_field']
...
def control_0():
    ...
    _last_outcome = command_create({'priority_field': _inputs['entry.priority_field'].get(), 'response_deadline_field': _inputs['entry.response_deadline_field'].get(), 'service_state_field': _inputs['entry.service_state_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'id': selected_value('collection.primary', 'id'), 'priority_field': _inputs['entry.priority_field'].get(), 'response_deadline_field': _inputs['entry.response_deadline_field'].get(), 'service_state_field': _inputs['entry.service_state_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Priority Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.priority_field'] = Entry(_root, width=38)
    _inputs['entry.priority_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Service State Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.service_state_field'] = Entry(_root, width=38)
    _inputs['entry.service_state_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Response Deadline Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.response_deadline_field'] = Entry(_root, width=38)
    _inputs['entry.response_deadline_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}, 'id': 'support-queue.lifecycle', 'input': {'steps': [{'arguments': {'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}, 'inputs': {'entry.primary': 'A', 'entry.priority_field': 'priority_field-value', 'entry.response_deadline_field': 'response_deadline_field-value', 'entry.service_state_field': 'service_state_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'B'}]}}, 'inputs': {'entry.primary': 'B', 'entry.priority_field': 'priority_field-value', 'entry.response_deadline_field': 'response_deadline_field-value', 'entry.service_state_field': 'service_state_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'priority_field': 'priority_field-value', 'response_deadline_field': 'response_deadline_field-value', 'service_state_field': 'service_state_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
