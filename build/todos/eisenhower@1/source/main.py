...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'importance_field', 'urgency_field', 'quadrant_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/eisenhower/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_EISENHOWER_STATE'
...
def command_create(arguments):
    ...
    if 'importance_field' not in arguments:
        return _failure('missing-importance_field')
    if 'urgency_field' not in arguments:
        return _failure('missing-urgency_field')
    if 'quadrant_field' not in arguments:
        return _failure('missing-quadrant_field')
    ...
    state['records'].append({'completed': False, 'id': state['next_id'], 'importance_field': arguments['importance_field'], 'quadrant_field': arguments['quadrant_field'], 'title': arguments['title'], 'urgency_field': arguments['urgency_field']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['importance_field'] = arguments['importance_field']
            record['quadrant_field'] = arguments['quadrant_field']
            record['title'] = arguments['title']
            record['urgency_field'] = arguments['urgency_field']
    persist_state()
    present_state()
    return _success()
...
def control_0():
    ...
    _last_outcome = command_create({'importance_field': _inputs['entry.importance_field'].get(), 'quadrant_field': _inputs['entry.quadrant_field'].get(), 'title': _inputs['entry.primary'].get(), 'urgency_field': _inputs['entry.urgency_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'id': selected_value('collection.primary', 'id'), 'importance_field': _inputs['entry.importance_field'].get(), 'quadrant_field': _inputs['entry.quadrant_field'].get(), 'title': _inputs['entry.primary'].get(), 'urgency_field': _inputs['entry.urgency_field'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Importance Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.importance_field'] = Entry(_root, width=38)
    _inputs['entry.importance_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Urgency Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.urgency_field'] = Entry(_root, width=38)
    _inputs['entry.urgency_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Quadrant Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.quadrant_field'] = Entry(_root, width=38)
    _inputs['entry.quadrant_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}, 'id': 'eisenhower.lifecycle', 'input': {'steps': [{'arguments': {'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}, 'inputs': {'entry.importance_field': 'importance_field-value', 'entry.primary': 'A', 'entry.quadrant_field': 'quadrant_field-value', 'entry.urgency_field': 'urgency_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'B', 'urgency_field': 'urgency_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'B', 'urgency_field': 'urgency_field-value'}]}}, 'inputs': {'entry.importance_field': 'importance_field-value', 'entry.primary': 'B', 'entry.quadrant_field': 'quadrant_field-value', 'entry.urgency_field': 'urgency_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'importance_field': 'importance_field-value', 'quadrant_field': 'quadrant_field-value', 'title': 'A', 'urgency_field': 'urgency_field-value'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
