...
APPLICATION_ID = 'uc://applications/recurring-chores@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'recurrence_field', 'assignee_field', 'next_due_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/recurring-chores/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_RECURRING_CHORES_STATE'
...
def command_create(arguments):
    ...
    if 'next_due_field' not in arguments:
        return _failure('missing-next_due_field')
    ...
    state['records'].append({'assignee_field': arguments['assignee_field'], 'completed': False, 'id': state['next_id'], 'next_due_field': arguments['next_due_field'], 'recurrence_field': arguments['recurrence_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            ...
            record['next_due_field'] = arguments['next_due_field']
...
def control_0():
    ...
    _last_outcome = command_create({'assignee_field': _inputs['entry.assignee_field'].get(), 'next_due_field': _inputs['entry.next_due_field'].get(), 'recurrence_field': _inputs['entry.recurrence_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'assignee_field': _inputs['entry.assignee_field'].get(), 'id': selected_value('collection.primary', 'id'), 'next_due_field': _inputs['entry.next_due_field'].get(), 'recurrence_field': _inputs['entry.recurrence_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    Label(_root, text='Assignee Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.assignee_field'] = Entry(_root, width=38)
    _inputs['entry.assignee_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Next Due Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.next_due_field'] = Entry(_root, width=38)
    _inputs['entry.next_due_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}, 'id': 'recurring-chores.lifecycle', 'input': {'steps': [{'arguments': {'assignee_field': 'assignee_field-value', 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}, 'inputs': {'entry.assignee_field': 'assignee_field-value', 'entry.next_due_field': 'next_due_field-value', 'entry.primary': 'A', 'entry.recurrence_field': 'recurrence_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': False, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'B'}]}}, 'inputs': {'entry.assignee_field': 'assignee_field-value', 'entry.next_due_field': 'next_due_field-value', 'entry.primary': 'B', 'entry.recurrence_field': 'recurrence_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'assignee_field': 'assignee_field-value', 'completed': True, 'id': 1, 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'assignee_field': 'assignee_field-value', 'next_due_field': 'next_due_field-value', 'recurrence_field': 'recurrence_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
