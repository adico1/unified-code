...
APPLICATION_ID = 'uc://applications/project-work-breakdown@1'
...
IDENTITY_FIELD = 'id'
DISPLAY_FIELDS = ['id', 'title', 'parent_task_field', 'milestone_field', 'progress_rollup_field', 'completed']
FILTER_FIELD = 'filter'
FILTERS = {'all': None, 'completed': {'equals': True, 'field': 'completed'}, 'open': {'equals': False, 'field': 'completed'}}
VISIBILITY = None
DEFAULT_STATE_PATH = '.unified-code-manual/project-work-breakdown/state.json'
STATE_ENVIRONMENT = 'UC_MANUAL_PROJECT_WORK_BREAKDOWN_STATE'
...
def command_create(arguments):
    ...
    if 'parent_task_field' not in arguments:
        return _failure('missing-parent_task_field')
    if 'milestone_field' not in arguments:
        return _failure('missing-milestone_field')
    if 'progress_rollup_field' not in arguments:
        return _failure('missing-progress_rollup_field')
    ...
    state['records'].append({'completed': False, 'id': state['next_id'], 'milestone_field': arguments['milestone_field'], 'parent_task_field': arguments['parent_task_field'], 'progress_rollup_field': arguments['progress_rollup_field'], 'title': arguments['title']})
    state['next_id'] += 1
    persist_state()
    present_state()
    return _success()
...
def command_update(arguments):
    ...
    for record in state['records']:
        if record['id'] == arguments['id']:
            record['milestone_field'] = arguments['milestone_field']
            record['parent_task_field'] = arguments['parent_task_field']
            record['progress_rollup_field'] = arguments['progress_rollup_field']
...
def control_0():
    ...
    _last_outcome = command_create({'milestone_field': _inputs['entry.milestone_field'].get(), 'parent_task_field': _inputs['entry.parent_task_field'].get(), 'progress_rollup_field': _inputs['entry.progress_rollup_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome

def control_1():
    global _last_outcome
    _last_outcome = command_update({'id': selected_value('collection.primary', 'id'), 'milestone_field': _inputs['entry.milestone_field'].get(), 'parent_task_field': _inputs['entry.parent_task_field'].get(), 'progress_rollup_field': _inputs['entry.progress_rollup_field'].get(), 'title': _inputs['entry.primary'].get()})
    _status.set(_last_outcome['error'] or 'ok')
    return _last_outcome
...
def build_interface():
    ...
    _metric_cards.clear()
    _root = Tk()
    _root.title('Project work breakdown')
    ...
    Label(_root, text='Parent Task Field').grid(row=1, column=0, sticky='w')
    _inputs['entry.parent_task_field'] = Entry(_root, width=38)
    _inputs['entry.parent_task_field'].grid(row=1, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Milestone Field').grid(row=2, column=0, sticky='w')
    _inputs['entry.milestone_field'] = Entry(_root, width=38)
    _inputs['entry.milestone_field'].grid(row=2, column=1, columnspan=3, sticky='ew')
    Label(_root, text='Progress Rollup Field').grid(row=3, column=0, sticky='w')
    _inputs['entry.progress_rollup_field'] = Entry(_root, width=38)
    _inputs['entry.progress_rollup_field'].grid(row=3, column=1, columnspan=3, sticky='ew')
...
def run_acceptance():
    cases = [{'expected': {'results': [{'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}, {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}], 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}, 'id': 'project-work-breakdown.lifecycle', 'input': {'steps': [{'arguments': {'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}, 'command': 'create'}, {'arguments': {'id': 1}, 'command': 'toggle'}, {'restart': True}]}}]
...
def self_test_interface():
    ...
    with tempfile.TemporaryDirectory(prefix='generated-stateful-gui-') as directory:
        ...
        root.withdraw()
        cases = [{'control': 'record.create', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}, 'inputs': {'entry.milestone_field': 'milestone_field-value', 'entry.parent_task_field': 'parent_task_field-value', 'entry.primary': 'A', 'entry.progress_rollup_field': 'progress_rollup_field-value'}}, {'control': 'record.update', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'B'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': False, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'B'}]}}, 'inputs': {'entry.milestone_field': 'milestone_field-value', 'entry.parent_task_field': 'parent_task_field-value', 'entry.primary': 'B', 'entry.progress_rollup_field': 'progress_rollup_field-value'}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.toggle', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}, 'state': {'filter': 'all', 'next_id': 2, 'records': [{'completed': True, 'id': 1, 'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}]}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'record.remove', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 2, 'records': []}}, 'state': {'filter': 'all', 'next_id': 2, 'records': []}}, 'selection': {'identity': 'collection.primary', 'index': 0}, 'setup': [{'arguments': {'milestone_field': 'milestone_field-value', 'parent_task_field': 'parent_task_field-value', 'progress_rollup_field': 'progress_rollup_field-value', 'title': 'A'}, 'command': 'create'}]}, {'control': 'filter.all', 'expected': {'outcome': {'error': None, 'result': {'filter': 'all', 'next_id': 1, 'records': []}}, 'state': {'filter': 'all', 'next_id': 1, 'records': []}}}, {'control': 'filter.open', 'expected': {'outcome': {'error': None, 'result': {'filter': 'open', 'next_id': 1, 'records': []}}, 'state': {'filter': 'open', 'next_id': 1, 'records': []}}}, {'control': 'filter.completed', 'expected': {'outcome': {'error': None, 'result': {'filter': 'completed', 'next_id': 1, 'records': []}}, 'state': {'filter': 'completed', 'next_id': 1, 'records': []}}}]
        for case in cases:
            outward.clear()
            reset_state()
