...
def _semantic_0(quantity, from_portions, to_portions):
    return (quantity * (to_portions / from_portions))

BINARY = {ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul, ast.Div: operator.truediv}
UNARY = {ast.UAdd: operator.pos, ast.USub: operator.neg}
FUNCTIONS = {'scale': _semantic_0}
...
def build_interface():
    ...
    root = Tk()
    root.title('Recipe quantity scaling calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='scale(quantity,from_portions,to_portions)')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('scale(quantity,from_portions,to_portions)')
