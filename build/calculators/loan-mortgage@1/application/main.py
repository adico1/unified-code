...
def _semantic_0(principal, rate, periods):
    return ((principal / periods) if (rate == 0) else ((principal * rate) / (1 - ((1 + rate) ** -(periods)))))

BINARY = {ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul, ast.Div: operator.truediv}
UNARY = {ast.UAdd: operator.pos, ast.USub: operator.neg}
FUNCTIONS = {'payment': _semantic_0}
...
def build_interface():
    ...
    root = Tk()
    root.title('Fixed-rate payment calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='payment(principal,rate,periods)')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('payment(principal,rate,periods)')
