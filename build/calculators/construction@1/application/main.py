...
def _semantic_0(length, width, height, waste):
    return (((length * width) * height) * (1 + waste))

BINARY = {ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul, ast.Div: operator.truediv}
UNARY = {ast.UAdd: operator.pos, ast.USub: operator.neg}
FUNCTIONS = {'material': _semantic_0}
...
def build_interface():
    ...
    root = Tk()
    root.title('Rectangular material-volume calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='material(length,width,height,waste)')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('material(length,width,height,waste)')
