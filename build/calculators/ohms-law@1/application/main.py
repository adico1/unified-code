...
def _semantic_0(current, resistance):
    return (current * resistance)

def _semantic_1(voltage, resistance):
    return (voltage / resistance)

def _semantic_2(voltage, current):
    return (voltage / current)

BINARY = {ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul, ast.Div: operator.truediv}
UNARY = {ast.UAdd: operator.pos, ast.USub: operator.neg}
FUNCTIONS = {'voltage': _semantic_0, 'current': _semantic_1, 'resistance': _semantic_2}
...
SELF_TEST_CONTROLS = [{'identity': 'function.voltage', 'label': 'V = I×R', 'row': 2, 'column': 0, 'action': 'append', 'value': 'voltage('}, {'identity': 'function.current', 'label': 'I = V÷R', 'row': 2, 'column': 1, 'action': 'append', 'value': 'current('}, {'identity': 'function.resistance', 'label': 'R = V÷I', 'row': 2, 'column': 2, 'action': 'append', 'value': 'resistance('}, {'identity': 'syntax.comma', 'label': ',', 'row': 2, 'column': 3, 'action': 'append', 'value': ','}, {'identity': 'syntax.right', 'label': ')', 'row': 2, 'column': 4, 'action': 'append', 'value': ')'}, {'identity': 'command.clear.compact', 'label': 'C', 'row': 2, 'column': 5, 'action': 'clear'}, {'identity': 'digit.7', 'label': '7', 'row': 3, 'column': 0, 'action': 'append', 'value': '7'}, {'identity': 'digit.8', 'label': '8', 'row': 3, 'column': 1, 'action': 'append', 'value': '8'}, {'identity': 'digit.9', 'label': '9', 'row': 3, 'column': 2, 'action': 'append', 'value': '9'}, {'identity': 'operator.expression.divide', 'label': '÷', 'row': 3, 'column': 3, 'action': 'append', 'value': '/'}, {'identity': 'operator.expression.multiply', 'label': '×', 'row': 3, 'column': 4, 'action': 'append', 'value': '*'}, {'identity': 'command.backspace', 'label': '⌫', 'row': 3, 'column': 5, 'action': 'backspace'}, {'identity': 'digit.4', 'label': '4', 'row': 4, 'column': 0, 'action': 'append', 'value': '4'}, {'identity': 'digit.5', 'label': '5', 'row': 4, 'column': 1, 'action': 'append', 'value': '5'}, {'identity': 'digit.6', 'label': '6', 'row': 4, 'column': 2, 'action': 'append', 'value': '6'}, {'identity': 'operator.expression.subtract', 'label': '−', 'row': 4, 'column': 3, 'action': 'append', 'value': '-'}, {'identity': 'operator.expression.add', 'label': '+', 'row': 4, 'column': 4, 'action': 'append', 'value': '+'}, {'identity': 'syntax.left', 'label': '(', 'row': 4, 'column': 5, 'action': 'append', 'value': '('}, {'identity': 'digit.1', 'label': '1', 'row': 5, 'column': 0, 'action': 'append', 'value': '1'}, {'identity': 'digit.2', 'label': '2', 'row': 5, 'column': 1, 'action': 'append', 'value': '2'}, {'identity': 'digit.3', 'label': '3', 'row': 5, 'column': 2, 'action': 'append', 'value': '3'}, {'identity': 'digit.0', 'label': '0', 'row': 5, 'column': 3, 'action': 'append', 'value': '0'}, {'identity': 'syntax.decimal', 'label': '.', 'row': 5, 'column': 4, 'action': 'append', 'value': '.'}, {'identity': 'command.evaluate', 'label': '=', 'row': 5, 'column': 5, 'action': 'evaluate'}]
...
def build_interface():
    ...
    root.geometry('720x350+440+220')
    root.configure(bg='#fff8dc')
    display = StringVar()
    mode_text = StringVar(value='signed real · volts · amperes · ohms')
    Entry(root, textvariable=display, font=('Menlo', 18), justify='right').grid(row=0, column=0, columnspan=6, sticky='nsew')
    Label(root, textvariable=mode_text, bg='#fff8dc', fg='#5c3d00').grid(row=1, column=0, columnspan=6, sticky='w')
    Button(root, text='V = I×R', command=lambda value='voltage(': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=2, column=0, sticky='nsew')
    Button(root, text='I = V÷R', command=lambda value='current(': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=2, column=1, sticky='nsew')
    Button(root, text='R = V÷I', command=lambda value='resistance(': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=2, column=2, sticky='nsew')
    Button(root, text=',', command=lambda value=',': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=2, column=3, sticky='nsew')
    Button(root, text=')', command=lambda value=')': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=2, column=4, sticky='nsew')
    Button(root, text='C', command=clear, bg='#ffffff', fg='#5c3d00', width=7).grid(row=2, column=5, sticky='nsew')
    Button(root, text='7', command=lambda value='7': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=3, column=0, sticky='nsew')
    Button(root, text='8', command=lambda value='8': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=3, column=1, sticky='nsew')
    Button(root, text='9', command=lambda value='9': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=3, column=2, sticky='nsew')
    Button(root, text='÷', command=lambda value='/': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=3, column=3, sticky='nsew')
    Button(root, text='×', command=lambda value='*': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=3, column=4, sticky='nsew')
    Button(root, text='⌫', command=backspace, bg='#ffffff', fg='#5c3d00', width=7).grid(row=3, column=5, sticky='nsew')
    Button(root, text='4', command=lambda value='4': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=4, column=0, sticky='nsew')
    Button(root, text='5', command=lambda value='5': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=4, column=1, sticky='nsew')
    Button(root, text='6', command=lambda value='6': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=4, column=2, sticky='nsew')
    Button(root, text='−', command=lambda value='-': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=4, column=3, sticky='nsew')
    Button(root, text='+', command=lambda value='+': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=4, column=4, sticky='nsew')
    Button(root, text='(', command=lambda value='(': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=4, column=5, sticky='nsew')
    Button(root, text='1', command=lambda value='1': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=5, column=0, sticky='nsew')
    Button(root, text='2', command=lambda value='2': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=5, column=1, sticky='nsew')
    Button(root, text='3', command=lambda value='3': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=5, column=2, sticky='nsew')
    Button(root, text='0', command=lambda value='0': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=5, column=3, sticky='nsew')
    Button(root, text='.', command=lambda value='.': append(value), bg='#ffffff', fg='#5c3d00', width=7).grid(row=5, column=4, sticky='nsew')
    Button(root, text='=', command=evaluate, bg='#ffffff', fg='#5c3d00', width=7).grid(row=5, column=5, sticky='nsew')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('signed real · volts · amperes · ohms')
