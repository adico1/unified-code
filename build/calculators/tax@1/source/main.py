...
def _semantic_0(income):
    return ((income * 0.1) if (min(income, 10000) == income) else (1000 + ((income - 10000) * 0.2)))

BINARY = {ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul, ast.Div: operator.truediv}
UNARY = {ast.UAdd: operator.pos, ast.USub: operator.neg}
FUNCTIONS = {'example_tax': _semantic_0}
...
def build_interface():
    ...
    root = Tk()
    root.title('Pinned example tax calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='example_tax(income) · 10% ≤10000, 20% above')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('example_tax(income) · 10% ≤10000, 20% above')
