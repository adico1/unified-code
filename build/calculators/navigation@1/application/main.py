...
def build_interface():
    ...
    root = Tk()
    root.title('Planar navigation calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='distance(x1,y1,x2,y2) · bearing(x1,y1,x2,y2)')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('distance(x1,y1,x2,y2) · bearing(x1,y1,x2,y2)')
