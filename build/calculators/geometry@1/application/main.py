...
def _semantic_0(radius):
    return (3.141592653589793 * (radius ** 2))

def _semantic_1(width, height):
    return (width * height)

BINARY = {ast.Add: operator.add, ast.Sub: operator.sub, ast.Mult: operator.mul, ast.Div: operator.truediv}
UNARY = {ast.UAdd: operator.pos, ast.USub: operator.neg}
FUNCTIONS = {'circle_area': _semantic_0, 'rectangle_area': _semantic_1}
...
def build_interface():
    ...
    root = Tk()
    root.title('Circle and rectangle area calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='circle_area(radius) · rectangle_area(width,height)')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('circle_area(radius) · rectangle_area(width,height)')
