...
def _semantic_0(*values):
    return statistics.mean(list(values))

def _semantic_1(*values):
    return statistics.median(list(values))

def _semantic_2(*values):
    return statistics.pstdev(list(values))

def _semantic_3(*values):
    return sum(*values)

def _semantic_4(*values):
    return min(*values)

def _semantic_5(*values):
    return max(*values)
...
FUNCTIONS = {'mean': _semantic_0, 'median': _semantic_1, 'pstdev': _semantic_2, 'sum': _semantic_3, 'minimum': _semantic_4, 'maximum': _semantic_5}
...
SELF_TEST_CONTROLS = [{'identity': 'function.mean', 'label': 'mean', 'row': 2, 'column': 0, 'action': 'append', 'value': 'mean('}, {'identity': 'function.median', 'label': 'median', 'row': 2, 'column': 1, 'action': 'append', 'value': 'median('}, {'identity': 'function.stdev', 'label': 'σ', 'row': 2, 'column': 2, 'action': 'append', 'value': 'pstdev('}, {'identity': 'function.sum', 'label': 'Σ', 'row': 2, 'column': 3, 'action': 'append', 'value': 'sum('}, {'identity': 'function.min', 'label': 'min', 'row': 2, 'column': 4, 'action': 'append', 'value': 'minimum('}, {'identity': 'function.max', 'label': 'max', 'row': 2, 'column': 5, 'action': 'append', 'value': 'maximum('}, {'identity': 'digit.7', 'label': '7', 'row': 3, 'column': 0, 'action': 'append', 'value': '7'}, {'identity': 'digit.8', 'label': '8', 'row': 3, 'column': 1, 'action': 'append', 'value': '8'}, {'identity': 'digit.9', 'label': '9', 'row': 3, 'column': 2, 'action': 'append', 'value': '9'}, {'identity': 'syntax.comma', 'label': ',', 'row': 3, 'column': 3, 'action': 'append', 'value': ','}, {'identity': 'syntax.right', 'label': ')', 'row': 3, 'column': 4, 'action': 'append', 'value': ')'}, {'identity': 'command.clear.compact', 'label': 'C', 'row': 3, 'column': 5, 'action': 'clear'}, {'identity': 'digit.4', 'label': '4', 'row': 4, 'column': 0, 'action': 'append', 'value': '4'}, {'identity': 'digit.5', 'label': '5', 'row': 4, 'column': 1, 'action': 'append', 'value': '5'}, {'identity': 'digit.6', 'label': '6', 'row': 4, 'column': 2, 'action': 'append', 'value': '6'}, {'identity': 'digit.1', 'label': '1', 'row': 4, 'column': 3, 'action': 'append', 'value': '1'}, {'identity': 'digit.2', 'label': '2', 'row': 4, 'column': 4, 'action': 'append', 'value': '2'}, {'identity': 'digit.3', 'label': '3', 'row': 4, 'column': 5, 'action': 'append', 'value': '3'}, {'identity': 'digit.0', 'label': '0', 'row': 5, 'column': 0, 'action': 'append', 'value': '0'}, {'identity': 'syntax.decimal', 'label': '.', 'row': 5, 'column': 1, 'action': 'append', 'value': '.'}, {'identity': 'operator.expression.subtract', 'label': '−', 'row': 5, 'column': 2, 'action': 'append', 'value': '-'}, {'identity': 'syntax.left', 'label': '(', 'row': 5, 'column': 3, 'action': 'append', 'value': '('}, {'identity': 'command.backspace', 'label': '⌫', 'row': 5, 'column': 4, 'action': 'backspace'}, {'identity': 'command.evaluate', 'label': '=', 'row': 5, 'column': 5, 'action': 'evaluate'}]

def build_interface():
    global display, mode_text
    root = Tk()
    root.title('Statistical Calculator')
    root.geometry('560x350+1150+470')
    root.configure(bg='#f4f1de')
    display = StringVar()
    mode_text = StringVar(value='descriptive statistics · comma-separated samples')
    Entry(root, textvariable=display, font=('Menlo', 18), justify='right').grid(row=0, column=0, columnspan=6, sticky='nsew')
    Label(root, textvariable=mode_text, bg='#f4f1de', fg='#3d405b').grid(row=1, column=0, columnspan=6, sticky='w')
    Button(root, text='mean', command=lambda value='mean(': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=2, column=0, sticky='nsew')
    Button(root, text='median', command=lambda value='median(': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=2, column=1, sticky='nsew')
    Button(root, text='σ', command=lambda value='pstdev(': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=2, column=2, sticky='nsew')
    Button(root, text='Σ', command=lambda value='sum(': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=2, column=3, sticky='nsew')
    Button(root, text='min', command=lambda value='minimum(': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=2, column=4, sticky='nsew')
    Button(root, text='max', command=lambda value='maximum(': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=2, column=5, sticky='nsew')
    Button(root, text='7', command=lambda value='7': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=3, column=0, sticky='nsew')
    Button(root, text='8', command=lambda value='8': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=3, column=1, sticky='nsew')
    Button(root, text='9', command=lambda value='9': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=3, column=2, sticky='nsew')
    Button(root, text=',', command=lambda value=',': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=3, column=3, sticky='nsew')
    Button(root, text=')', command=lambda value=')': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=3, column=4, sticky='nsew')
    Button(root, text='C', command=clear, bg='#ffffff', fg='#3d405b', width=7).grid(row=3, column=5, sticky='nsew')
    Button(root, text='4', command=lambda value='4': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=4, column=0, sticky='nsew')
    Button(root, text='5', command=lambda value='5': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=4, column=1, sticky='nsew')
    Button(root, text='6', command=lambda value='6': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=4, column=2, sticky='nsew')
    Button(root, text='1', command=lambda value='1': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=4, column=3, sticky='nsew')
    Button(root, text='2', command=lambda value='2': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=4, column=4, sticky='nsew')
    Button(root, text='3', command=lambda value='3': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=4, column=5, sticky='nsew')
    Button(root, text='0', command=lambda value='0': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=5, column=0, sticky='nsew')
    Button(root, text='.', command=lambda value='.': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=5, column=1, sticky='nsew')
    Button(root, text='−', command=lambda value='-': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=5, column=2, sticky='nsew')
    Button(root, text='(', command=lambda value='(': append(value), bg='#ffffff', fg='#3d405b', width=7).grid(row=5, column=3, sticky='nsew')
    Button(root, text='⌫', command=backspace, bg='#ffffff', fg='#3d405b', width=7).grid(row=5, column=4, sticky='nsew')
    Button(root, text='=', command=evaluate, bg='#ffffff', fg='#3d405b', width=7).grid(row=5, column=5, sticky='nsew')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('descriptive statistics · comma-separated samples')
