...
FUNCTIONS = {'slope': _semantic_0, 'intercept': _semantic_1}
...
def build_interface():
    ...
    root = Tk()
    root.title('Two-point regression calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='slope(x1,y1,x2,y2) · intercept(x1,y1,x2,y2)')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('slope(x1,y1,x2,y2) · intercept(x1,y1,x2,y2)')
