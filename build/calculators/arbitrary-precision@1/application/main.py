...
def build_interface():
    ...
    root = Tk()
    root.title('Exact Python-integer power calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='exact_power(base,exponent) · Python integer')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('exact_power(base,exponent) · Python integer')
...
def self_test_interface(root):
    ...
    reset_interface()
    display.set('exact_power(2,100)')
    state['expression'] = ''
    widgets = [item for item in root.grid_slaves(row=5, column=2) if item.winfo_class() == 'Button']
    widgets[0].invoke()
    results.append(display.get() == '1267650600228229401496703205376')
    reset_interface()
