...
def build_interface():
    ...
    root = Tk()
    root.title('Complex coordinate magnitude and phase calculator')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='magnitude(real,imag) · phase(real,imag)')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('magnitude(real,imag) · phase(real,imag)')
