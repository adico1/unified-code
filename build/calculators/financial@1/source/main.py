...
def _semantic_0(rate, periods, present):
    return (-((present / periods)) if (rate == 0) else -(((rate * present) / (1 - ((1 + rate) ** -(periods))))))

def _semantic_1(rate, periods, periodic_payment, present):
    return (-((present + (periodic_payment * periods))) if (rate == 0) else -(((present * ((1 + rate) ** periods)) + (periodic_payment * ((((1 + rate) ** periods) - 1) / rate)))))

def _semantic_2(rate, periods, periodic_payment):
    return (-((periodic_payment * periods)) if (rate == 0) else -(((periodic_payment * (1 - ((1 + rate) ** -(periods)))) / rate)))
...
FUNCTIONS = {'payment': _semantic_0, 'future_value': _semantic_1, 'present_value': _semantic_2}
...
SELF_TEST_CONTROLS = [{'identity': 'function.pmt', 'label': 'PMT', 'row': 2, 'column': 0, 'action': 'append', 'value': 'payment('}, {'identity': 'function.fv', 'label': 'FV', 'row': 2, 'column': 1, 'action': 'append', 'value': 'future_value('}, {'identity': 'function.pv', 'label': 'PV', 'row': 2, 'column': 2, 'action': 'append', 'value': 'present_value('}, {'identity': 'syntax.comma', 'label': ',', 'row': 2, 'column': 3, 'action': 'append', 'value': ','}, {'identity': 'syntax.right', 'label': ')', 'row': 2, 'column': 4, 'action': 'append', 'value': ')'}, {'identity': 'digit.7', 'label': '7', 'row': 3, 'column': 0, 'action': 'append', 'value': '7'}, {'identity': 'digit.8', 'label': '8', 'row': 3, 'column': 1, 'action': 'append', 'value': '8'}, {'identity': 'digit.9', 'label': '9', 'row': 3, 'column': 2, 'action': 'append', 'value': '9'}, {'identity': 'operator.expression.divide', 'label': '÷', 'row': 3, 'column': 3, 'action': 'append', 'value': '/'}, {'identity': 'command.clear.compact', 'label': 'C', 'row': 3, 'column': 4, 'action': 'clear'}, {'identity': 'digit.4', 'label': '4', 'row': 4, 'column': 0, 'action': 'append', 'value': '4'}, {'identity': 'digit.5', 'label': '5', 'row': 4, 'column': 1, 'action': 'append', 'value': '5'}, {'identity': 'digit.6', 'label': '6', 'row': 4, 'column': 2, 'action': 'append', 'value': '6'}, {'identity': 'operator.expression.multiply', 'label': '×', 'row': 4, 'column': 3, 'action': 'append', 'value': '*'}, {'identity': 'command.backspace', 'label': '⌫', 'row': 4, 'column': 4, 'action': 'backspace'}, {'identity': 'digit.1', 'label': '1', 'row': 5, 'column': 0, 'action': 'append', 'value': '1'}, {'identity': 'digit.2', 'label': '2', 'row': 5, 'column': 1, 'action': 'append', 'value': '2'}, {'identity': 'digit.3', 'label': '3', 'row': 5, 'column': 2, 'action': 'append', 'value': '3'}, {'identity': 'operator.expression.subtract', 'label': '−', 'row': 5, 'column': 3, 'action': 'append', 'value': '-'}, {'identity': 'operator.expression.add', 'label': '+', 'row': 5, 'column': 4, 'action': 'append', 'value': '+'}, {'identity': 'digit.0', 'label': '0', 'row': 6, 'column': 0, 'action': 'append', 'value': '0'}, {'identity': 'syntax.decimal', 'label': '.', 'row': 6, 'column': 1, 'action': 'append', 'value': '.'}, {'identity': 'syntax.left', 'label': '(', 'row': 6, 'column': 2, 'action': 'append', 'value': '('}, {'identity': 'operator.expression.power', 'label': 'xʸ', 'row': 6, 'column': 3, 'action': 'append', 'value': '**'}, {'identity': 'command.evaluate', 'label': '=', 'row': 6, 'column': 4, 'action': 'evaluate'}]

def build_interface():
    global display, mode_text
    root = Tk()
    root.title('Financial Calculator')
    root.geometry('520x330+610+470')
    root.configure(bg='#edf6f9')
    display = StringVar()
    mode_text = StringVar(value='cash-flow functions · decimal real')
    Entry(root, textvariable=display, font=('Menlo', 18), justify='right').grid(row=0, column=0, columnspan=5, sticky='nsew')
    Label(root, textvariable=mode_text, bg='#edf6f9', fg='#003049').grid(row=1, column=0, columnspan=5, sticky='w')
    Button(root, text='PMT', command=lambda value='payment(': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=2, column=0, sticky='nsew')
    Button(root, text='FV', command=lambda value='future_value(': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=2, column=1, sticky='nsew')
    Button(root, text='PV', command=lambda value='present_value(': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=2, column=2, sticky='nsew')
    Button(root, text=',', command=lambda value=',': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=2, column=3, sticky='nsew')
    Button(root, text=')', command=lambda value=')': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=2, column=4, sticky='nsew')
    Button(root, text='7', command=lambda value='7': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=3, column=0, sticky='nsew')
    Button(root, text='8', command=lambda value='8': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=3, column=1, sticky='nsew')
    Button(root, text='9', command=lambda value='9': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=3, column=2, sticky='nsew')
    Button(root, text='÷', command=lambda value='/': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=3, column=3, sticky='nsew')
    Button(root, text='C', command=clear, bg='#ffffff', fg='#003049', width=7).grid(row=3, column=4, sticky='nsew')
    Button(root, text='4', command=lambda value='4': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=4, column=0, sticky='nsew')
    Button(root, text='5', command=lambda value='5': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=4, column=1, sticky='nsew')
    Button(root, text='6', command=lambda value='6': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=4, column=2, sticky='nsew')
    Button(root, text='×', command=lambda value='*': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=4, column=3, sticky='nsew')
    Button(root, text='⌫', command=backspace, bg='#ffffff', fg='#003049', width=7).grid(row=4, column=4, sticky='nsew')
    Button(root, text='1', command=lambda value='1': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=5, column=0, sticky='nsew')
    Button(root, text='2', command=lambda value='2': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=5, column=1, sticky='nsew')
    Button(root, text='3', command=lambda value='3': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=5, column=2, sticky='nsew')
    Button(root, text='−', command=lambda value='-': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=5, column=3, sticky='nsew')
    Button(root, text='+', command=lambda value='+': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=5, column=4, sticky='nsew')
    Button(root, text='0', command=lambda value='0': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=6, column=0, sticky='nsew')
    Button(root, text='.', command=lambda value='.': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=6, column=1, sticky='nsew')
    Button(root, text='(', command=lambda value='(': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=6, column=2, sticky='nsew')
    Button(root, text='xʸ', command=lambda value='**': append(value), bg='#ffffff', fg='#003049', width=7).grid(row=6, column=3, sticky='nsew')
    Button(root, text='=', command=evaluate, bg='#ffffff', fg='#003049', width=7).grid(row=6, column=4, sticky='nsew')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('cash-flow functions · decimal real')
...
def self_test_interface(root):
    ...
    reset_interface()
    display.set('payment(0,10,1000)')
    state['expression'] = ''
    widgets = [item for item in root.grid_slaves(row=6, column=4) if item.winfo_class() == 'Button']
    widgets[0].invoke()
    results.append(display.get() == '-100')
    reset_interface()
