...
def _semantic_0(mass_kg, height_m):
...
def build_interface():
    ...
    root = Tk()
    root.title('BMI arithmetic calculator (not medical advice)')
    root.geometry('620x380+80+80')
    root.configure(bg='light green')
    display = StringVar()
    mode_text = StringVar(value='bmi(mass_kg,height_m) · arithmetic only')
...
def reset_interface():
    ...
    state['expression'] = '1'
    present_display('1')
    mode_text.set('bmi(mass_kg,height_m) · arithmetic only')
