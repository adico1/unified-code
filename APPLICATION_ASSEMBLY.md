# Five Applications, One Assembly

This milestone makes application generation—not bootstrap infrastructure—the
observable product. Five independent JSON seeds are the sole authority for five
complete generated installations:

```bash
uc assemble seed/application_suite.json \
  --output /tmp/unified-app-suite \
  --build \
  --install \
  --verify \
  --gauntlet-depths 10
```

The operation validates every seed, derives a canonical specification and plan,
generates seven Thing v2 stages plus runtime boundaries, tests, manifests,
documentation and entry points, builds and tests the trees, installs isolated
copies, executes restart scenarios, compares browser and headless replay, and
atomically publishes only after all ten depths pass.

For these Python/browser targets, build means parsing every generated Python
source and executable plus running the JavaScript host syntax gate before tests
or installation. Acceptance subprocesses enforce each seed's declared deadline.
No wheel or external package download is required.

## One generic language

The permanent compiler recognizes four generic audited capability families:

- document bytes and text mutation through an authorized OUTWARD root;
- bounded numeric operation tables;
- expression evaluation through a pinned generated-library interface;
- deterministic event/tick world transitions with presentation adapters.

Application names, commands, fields, validation values, persistence identity,
numeric operation names, event names, geometry and acceptance results live in
the seeds. They do not select application-named generator branches.

The anti-overfitting proof derives vocabulary from all five seeds, injects every
derived term into each permanent compiler/boilerplate surface, and requires
detection. A cloned suite also renames all five identities, operation names,
events, dependency references, and world roles before generating and executing
the applications again.

Each generated public stage is `part(thing) -> thing`. Generated stage and
composition files have zero explicit conditions, loops, matching,
comprehensions or exception handling. Physical control flow remains in the
audited generated capability runtime and process/browser boundaries.

## Generated products

| Seed | Product proof |
|---|---|
| `file_reader.json` | content, deterministic metadata, empty/missing/unavailable/encoding distinction and path authority |
| `file_editor.json` | preview, replace/append/insert/delete, atomic save, rejection preservation and process restart |
| `math_library.json` | reusable Standard Ten library interface with ten required bounded operations |
| `calculator.json` | precedence, parentheses, unary negative, division, invalid token, overflow and pinned library identity |
| `pong_game.json` | persisted event/tick state, collision/point transition, exact replay and Node/browser versus Python/headless equality |

Every tree contains canonical specification, application plan, seven stages,
domain specialization data, composition, audited runtime, CLI entry point,
installation/dependency/evidence manifests, generated tests, acceptance data,
documentation and deterministic identity. Browser targets also contain
generated HTML, CSS and JavaScript.

The installed runtime reads no source seed, registry or repository module.
Manual application runtime and manual application-test line counts are both
recorded as zero.

## Ten depths

The assembly report contains one nonempty report per application for:

1. seed/schema validity;
2. seed-to-spec fidelity;
3. spec-to-plan fidelity;
4. generation and provenance;
5. Standard Ten structure;
6. boundary authority and failure isolation;
7. behavior and persistence;
8. determinism and dependency identity;
9. mutation, differential presentation and performance boundary;
10. assembly, installation, execution and restart.

Every depth reports executed, passed, failed, verdict and ordered evidence.
Any zero denominator or failed application prevents publication.

## Honest boundary

This is a bounded application language, not a proof of arbitrary programs or
all GUI toolkits and hardware. Pong proves one deterministic generated browser
adapter and headless equivalence. Stage‑0, name-to-manifestation and the
root-seed fixed point remain separate support tracks.
