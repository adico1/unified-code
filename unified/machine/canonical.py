...
_EV_KEEP = re.compile(
    ...
    r"|eval:"
    # feature result marks: name:ok / name:error:... / name:missing-input
    r"(?!load:|encode:|decode:|compile:|validate:|machine:|prepare_|generate:|write:|boundary:load)"
    r"[a-z_][a-z0-9_]*:(?:ok|skipped|error:[^\s]+|missing-input|missing-text|missing-document)"
    r")"
)
