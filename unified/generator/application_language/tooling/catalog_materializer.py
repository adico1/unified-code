...
OUTPUT = ROOT / "build" / "application-language" / "authority"
...
def resolve_prototype(reference, seed_root=SEED_ROOT):
    ...
    path = (seed_root / relative).resolve(strict=True)
...
def materialize_profile(profile, seed_root=SEED_ROOT):
    derivation = profile["derivation"]
    if set(derivation) != {"prototype", "patch"}:
        raise ValueError("invalid-derived-seed")
    prototype, prototype_path = resolve_prototype(
        derivation["prototype"], seed_root
    )
        ...
        results.append(
            {
                ...
                "source_seed": "catalog.seed.json#" + profile["identity"],
