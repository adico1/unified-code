...
SUITE = SEED_ROOT / "suite.seed.json"
CATALOG = SEED_ROOT / "catalog.seed.json"
COMPILER = COMPILER_ROOT / "seed_compiler.py"
COMPILER_SOURCES = tuple(sorted(COMPILER_ROOT.glob("*.py")))
BUILD = ROOT / "build" / "application-language"
PUBLIC_OPERATION = ROOT / "unified" / "generator" / "cli.py"
...
@lru_cache(maxsize=1)
def load_compiler():
    from .. import seed_compiler

    return seed_compiler
...
def catalog_product_profiles():
    document = json.loads(CATALOG.read_text(encoding="utf-8"))
    return {
        profile["product_identity"]: profile["identity"]
        for family in document.get("families", ())
        for profile in family.get("profiles", ())
        if profile.get("status") == "proven"
    }
...
@lru_cache(maxsize=1)
def catalog_profiles_by_identity():
    document = json.loads(CATALOG.read_text(encoding="utf-8"))
    return {
...
def application_descriptor(item, groups):
    seed_path = Path(item.get("seed_path", SEED_ROOT / item["seed"])).resolve(
        strict=True
    )
    leaf = json.loads(seed_path.read_text(encoding="utf-8"))
    identity = coordinates(leaf)
    profiles = catalog_product_profiles()
    try:
    ...
    return {
        ...
        "source_seed": item.get("source_seed", item["seed"]),
        "profile_identity": item.get(
            "profile_identity", profiles[identity["canonical_identity"]]
        ),
        "seed_path": seed_path,
        "identity": identity,
    }
                ...
                product = (
                    ...
                    if products is not None
                        ...
                        if "derivation" in profile
                            ...
                            (SEED_ROOT / (seed_reference or "")).read_text()
                        )
                    )
                )
...
def compile_application_pair_worker(request):
    ...
    def compile_one(application, build_root):
        ...
        source_seed = application["source_seed"]
        if source_seed.startswith("catalog.seed.json#"):
            profile = catalog_profiles_by_identity()[
                application["profile_identity"]
            ]
            leaf_document, prototype = materialize_profile(profile)
            resolved_seed, authorities = compiler.resolve_seed_document(
                prototype, leaf_document
            )
        else:
...
def verify_cross_family_composition(generated, compiler):
    ...
    return {
        "applications": reports,
        "shared_expression_authority": (
            "unified/generator/application_language/semantic_expression.py"
        ),
        "mutations": {
...
def write_report(
    ...
    public_operation_bytes = PUBLIC_OPERATION.read_bytes()
    report = {
        "format": "manual-seed-assembly-report-1",
        "operation": "uc assemble",
        "applications": [
            {
                "id": item["id"],
                ...
                "registry_seed_sha256": item["registry_seed_sha256"],
        ...
        "build_time_shared": {
            **{
            ...
            "unified/generator/cli.py": {
                "sha256": digest(public_operation_bytes),
                "lines": len(public_operation_bytes.splitlines()),
            },
        },
...
def verify_seed_graph(compiler):
    ...
    try:
        ...
        root_path = SEED_ROOT / "bases" / "בלי_מה.seed.json"
...
def generate_in_stage(*, self_test, build_root, quiet=False):
    build_root = Path(build_root)
...
def main(argv=None):
    ...
    if arguments.build_root is not None:
        generate_in_stage(
            self_test=not arguments.generate_only,
