"""Compile unified application declarations into specialized Python trees."""
    ...
    if "plot" in routes:
        ...
        lines.extend(
            [
                ...
                f"        canvas.create_line(*coordinates, fill={accent!r}, width={rendering['plot_line_width']!r})",
...
def render_expression_source(seed):
...
def compile_expression_declaration(seed):
    tree = ast.parse(render_expression_source(seed))
...
def compile_declaration(seed):
    compilers = {
        "expression": compile_expression_declaration,
        "stateful": compile_stateful_declaration,
        "simulation": compile_simulation_declaration,
    }
    compiler = compilers.get(declaration_form(seed))
    if compiler is None:
        raise ValueError("declaration-language")
...
def render_declaration_source(seed):
    renderers = {
        "expression": render_expression_source,
        "stateful": render_stateful_source,
        "simulation": render_simulation_source,
    }
    renderer = renderers.get(declaration_form(seed))
    if renderer is None:
        raise ValueError("declaration-language")
